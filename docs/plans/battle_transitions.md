...
(§6) — tracked by `docs/current_plan_battle_completion.md` Stage 5's remaining box, NOT here.
ARCHIVED 2026-08-07 (maintainer direction) — this file is history.
...
- [x] Hoist `call InitBattleVariables` out of `InitBattleCanvas` (:205) to before :164
      (trainer) and :182 (wild) — restores pret `InitBattleCommon:32`, keeps
      music-before-transition.
- [x] Insert `call DoBattleTransitionAndInitBattleVariables` after `ReadTrainer` (:172) and
      after `LoadEnemyMonData` (:187) — pret's exact two call sites.
- [x] Move `LoadFontTilePatterns`/`LoadTextBoxTilePatterns` + `InitBattleCanvas` to AFTER the
...
- [x] Transition entry performs the flat-canvas switch (replaces pret's implicit "BG is already
...
- [x] Stage-1 build verifies the `W_TILEMAP` snapshot is current and aligned at encounter
...
- [x] `BattleTransition_CopyTiles1` inner count = **`(SCREEN_HEIGHT−1)/2` = 12** (pret's
      hardcoded `ld c, 8`; on GB `H/2−1 == (H−1)/2 == 8` — collision).
- [x] `BattleTransition_CopyTiles2` outer count = **`SCREEN_WIDTH/2−1` = 19** (pret's
      `ld c, SCREEN_HEIGHT/2` = 9 was really W/2−1 — collision). Inner column length =
      `SCREEN_HEIGHT` = 25.
- [x] Both keep pret's src/dst swap semantics (after each copy: new_src = old_dst + offset,
...
- [x] Calls: T1(src 0,11 → dst 0,12, offset −2 rows); T1(src 0,13 → dst 0,12, +2 rows);
...
- [x] Outer loop: **20 iterations** (pret 9). Columns step every iteration; rows step only on
...
- [x] *(measured)* Simulated with exact swap semantics: screen all-black exactly at iteration
...
- [x] Calls: T1(src 0,23 → dst 0,24, −2 rows); T1(src 0,1 → dst 0,0, +2 rows);
      T2(col 38 → 39, −2); T2(col 1 → 0, +2). Black seeds at center rows 12/13, cols 19/20.
- [x] Outer: **20 iterations**; rows on Bresenham-13/20 **{1,3,4,6,7,9,10,12,13,15,16,18,19}**;
...
- [x] Verbatim translation scales — counts are already `SCREEN_WIDTH/2` (=20 per row-pass) and
...
- [x] **Two columns per side per iteration** (cols 2i, 2i+1 even rows from the left; cols
      39−2i, 38−2i odd rows from the right) → 20 iterations × 3f = 60 frames = GB duration and
      the same per-step coverage fraction.
- [x] Odd height: the per-column fill count splits — **13** for even-row passes (rows
...
- [x] pret's `inc c`/`dec c` leg arithmetic silently encodes **W−H=2**; on 40×25 it breaks
...
- [x] **Zero-count legs must be skipped** (guard `test ecx,ecx / jz`): the final degenerate lap
      has an up-leg of 0 — pret's 8-bit `dec c` idiom would loop 256×. DEVIATION
      class=projection for the D-generalization + guard.
- [x] `wInwardSpiralUpdateScreenCounter` init 7 → **19** (≈ same cells-per-frame rate:
      1000/19 vs 360/7).
- [x] *(measured)* Simulator reproduces pret on GB exactly — 359/360 cells written, (8,9) left
...
- [x] *(measured)* pret's FSM misbehaves at screen edges even on GB: 19 OOB writes (into the
...
- [x] Port: **bounds-guarded FSM** — an out-of-bounds neighbor read reports "not $ff" (forcing
...
- [x] Parameters: start `hlcoord 20, 13` (pret 10,10 = W/2, H/2+1), initial direction 3,
...
- [x] `wOutwardSpiralTileMapPointer` as 16-bit GB offset (§3).

### Circle (idx 010) / DoubleCircle (idx 000) — regenerated arc tables

- [x] **Tier-1 generated data**: `tools/generators/gen_battle_transition_arcs.py` →
      `assets/battle_transition_arcs.inc`, wired into `make assets`. Never hand-author the
      bytes.
- [x] Generator algorithm *(prototyped + verified this session)*: **10 wedges per half** (keeps
...
- [x] Encoding = pret's `Circle_Sub3` format, unchanged: per row `run` byte then `skip|0`
...
- [x] *(measured)* Prototype output: all 20 wedges encodable (rows contiguous, one interval
...
- [x] `BattleTransition_FlashScreen_` + `FlashScreenPalettes`: bytes
...
- [x] `GetBattleTransitionID_{WildOrTrainer,CompareLevels,IsDungeonMap}` verbatim
      (`wCurOpponent`/`wPartyMon1HP`/`wCurEnemyLevel`/`wCurMap` all modeled; `wLinkState`
      exists — link branch never taken, keep it faithful).
- [x] **Preserve the documented bug** (`docs/bugs_and_glitches.md:37` "Battle transitions fail
...
- [x] `BattleTransition` prologue: Delay3 → (hWY: see §3, `hide_window`) →
...
- [x] **Stage 1 — skeleton (cheapest visible win):** wrapper + `BattleTransition`
...
- [x] **Stage 2 — stripes** (vertical verbatim, horizontal 2-col).
- [x] **Stage 3 — Shrink + Split** (schedules in §4).
- [x] **Stage 4 — spirals** (D-generalized inward; bounds-guarded outward) + WRAM vars.
- [x] **Stage 5 — Circle + DoubleCircle** (arc generator) + FlashScreen.
- [x] **Stage 6 — selection logic** + `dungeon_maps` data + scripted-battle BUG tag + trainer
      OAM spare verification (2-NPC map).
- [x] **Stage 7 — gates + goldens closure** (§6).

---

## 6. Verification & gates

- [x] Mirror path `dos_port/src/engine/battle/battle_transitions.asm`; add to
...
- [x] `fidelity_gate` faithdiffs every label in the file. Expected ADDED HAL calls
...
- [x] DEVIATION/BUG annotations: all five fields, classes projection / timing / HAL /
      data-model only; no `;` or `}` inside values.
- [x] **Golden impact** *(measured)*: scenarios 14/16 (`AUTOKEY_DUMP_FRAME=300`) stay safe
