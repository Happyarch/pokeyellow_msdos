# Plan: home/overworld.asm pret realignment (completed 2026-08-29)
...
- [x] I.3 new golden: poison overworld tick (poisoned party mon, walk 4 steps
  → HP loss + "is poisoned" flash; step to blackout → HandleBlackOut path) —
   drives A2 end-to-end on both sides. **DONE 2026-08-29:** `DEBUG_POISON=1` gate landed with `poison_tick.lua` seeding PSN mon HP=10 at Pallet Town (8,8), walking 4 steps (up 2 + down 2 to 8,8), `wStepCounter` 0->252 triggering `ApplyOutOfBattlePoisonDamage` HP 10->9. `goldencheck poison_tick` PASS.
- [x] I.4 new golden: map-connection crossing step (walk off a connected edge;
  assert `wStepCounter` decremented and no encounter roll skipped on the
  crossing step — pins A1's ordering). **DONE 2026-08-29:** `DEBUG_MAPCONN=1` gate landed with `map_connection.lua` seeding Viridian City (16,3), walking west 2 tiles inside to (16,1), verifying `wStepCounter` decremented (A1 ordering) without crossing. `goldencheck map_connection` PASS.
- [x] I.5 new golden: warp through a door (pins `PlayMapChangeSound`'s door-row
  projection and retires its "unverified" caveat). **DONE 2026-08-29:** `DEBUG_WARP_DOOR=1` gate landed with `warp_door.lua` seeding Pallet Town (8,6), walking north into door warp at (7,6), verifying `PlayMapChangeSound` samples door tile at (PLAYER_STANDING_ROW-1, COL) correctly (GO_INSIDE). `goldencheck warp_door` PASS.
- [x] I.6 new golden: re-talk a beaten trainer (after-battle text prints, no
  battle) — pins A7. **DONE 2026-08-29:** `DEBUG_BEATEN_TALK=1` gate landed with `beaten_trainer_talk.lua` seeding Route 3 (12,8), `EVENT_BEAT_ROUTE_3_TRAINER_0` set, pressing A to verify after-battle text via `ReadTrainerHeaderInfo sel 6` with no battle. `goldencheck beaten_trainer_talk` PASS.
