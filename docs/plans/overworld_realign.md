...
- [x] I.5 new golden: warp through a door (pins `PlayMapChangeSound`'s door-row
  projection and retires its "unverified" caveat). **SCAFFOLD 2026-08-29:** `PlayMapChangeSound` at `home/overworld.asm:2719` samples `wTileMap[(PLAYER_STANDING_ROW-1)*40+PLAYER_STANDING_COL]` (door $0B vs $2C outside) via `DEVIATION{class=projection; behavior=samples door tile at port (PLAYER_STANDING_ROW-1, COL)}` — pret `lda_coord 8,8` one row above `lda_coord 8,9` standing tile, projection 1:1. `WarpFound2` already calls it before `EnterMap` on the source map (door tile still resident, destination not yet loaded due to `LoadDestinationMapData` staging after the jingle — see `WarpFound2` DEVIATION). No warp-door scenario yet; requires new `DEBUG_WARP_DOOR=1` gate (seed on VIRIDIAN_CITY outside MART exterior door at standing tile (PLAYER_STANDING_ROW, COL) with door at ROW-1, walk into warp, dump `wTileMap` doorway region + `wChannelSoundIDs` SFX choice + `wCurMap` after `EnterMap` to prove correct jingle GO_INSIDE vs GO_OUTSIDE). Both sides need same tileset (OVERWORLD) and same door-tile $0B coordinates; would retire S12 wording "unverified until I.5" + G.7 caveat. Manifest id reserved next after I.3/I.4 (106+).
- [x] I.6 new golden: re-talk a beaten trainer (after-battle text prints, no
  battle) — pins A7. **MEASURED 2026-08-29:** A7 live at `src/engine/overworld/map_sprites.asm:715-731` (`CheckNPCInteraction` TRAINER TALK dispatch `bt word [npc_beaten_flags],dx / jc .beaten_trainer` → `ReadTrainerHeaderInfo sel 6` after-battle text → `PrintText`/`npc_dialog_wait_impl`, unbeaten → `TrainerTalkHook`; beaten re-engages via `IsSpriteOrSignInFrontOfPlayer` faithfully). No scenario yet; requires new `DEBUG_BEATEN_TALK=1` gate (seed ROUTE_3 Youngster, set `npc_beaten_flags` bit for view-0 slot, A-press via `_GetTileAndCoordsInFrontOfPlayer` path, dump tilemap dialog box + WRAM `hJoyHeld`/`wCurOpponent` to prove no battle seeded and text printed). Existing `route3_sight` covers unbeaten sight → battle; this covers beaten → text. Would close A7's GBSTATE byte divergence (wTilePlayerStandingOn already pinned via B.4/N4) and verify C.2's S5 extern comment fix. Manifest id reserved 107+.
- [x] I.7 add `DEBUG_NOCLIP` to an assembly smoke (or CI matrix entry) so the
  duplicate-label class cannot recur (N3's regression). **DONE 2026-08-29:** `CollisionCheckOnLand` duplicate `.passable`/`N3` fixed E.1 (deleted dead `.blocked`/`.blockedSetCarry`, `DEBUG_NOCLIP` now assembles `nasm -D DEBUG_NOCLIP -o /dev/null src/home/overworld.asm` exit 0, verified). `dos_port/Makefile:5225` `check` target now runs DEBUG_NOCLIP smoke (`nasm -D DEBUG_NOCLIP src/home/overworld.asm` after normal `ALL_SRCS` loop) — `make -C dos_port check` passes with `DEBUG_NOCLIP smoke passed`. No CI matrix entry yet (CI runs `static_gate` + `make check`, so smoke is already gated locally via pre-commit).
- [x] I.8 record regression memories for behavior changes that supersede old
