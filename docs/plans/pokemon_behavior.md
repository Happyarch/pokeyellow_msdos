...
## Stage 4 — `status_screen.asm`: the summary screen — [~] (PAGES 1 & 2 DONE + live-verified)

<!-- ===================== STAGE 4 STATUS (2026-07-03) =====================
PAGES 1 & 2 (StatusScreen / StatusScreen2) are ported, linked, and LIVE-VERIFIED
in DOSBox-X. The mon front-pic, the cry, and the two caller wirings remain.

PAGE 2 (StatusScreen2) — DONE + live-verified (FRAME.BIN via
`make DEBUG_STATUS_PAGE2=1`): THUNDERSHOCK 30/30 / GROWL 40/40 move+PP list,
EXP POINTS 1225, LEVEL UP 91 to :L6, blank-slot "--". Ported faithfully incl.
StatusScreen_PrintPP / StatusScreen_ClearName; the .PrintPP loop drives GetMaxPP
per move. New dependency GetMaxPP + AddBonusPP + GetSelectedMoveOffset ported to
src/engine/pokemon/get_max_pp.asm (its pret home is item_effects.asm; split out so
it links independently of the deferred item-USE dispatch, like knows_hm_move.asm).
  KEY SM83 FLAG/WIDTH GOTCHA (per CLAUDE.md "Preserve Flags"): AddBonusPP's PP-Up
  loop is an 8-bit do-while — for a mon with 0 PP-Ups (c=0) it loops 256× but adds
  a constant to the 8-bit accumulator, a net no-op (mod 256) → base PP unchanged,
  correct AND bounded. The x86 port MUST use an 8-bit `dec cl` (not `dec ecx`), or
  c=0 spins ~4 billion times (this actually hung the first page-2 build until
  fixed). Get-max scratch added to gb_memmap (wPPUpCountAndMaxPP, wUsingPPUp,
  wPartyMon1Moves/wEnemyMon1Moves/wBoxMon1Moves/wDayCareMonMoves — sym-verified).

TWO-TIER / GENERATOR FIX (user caught this): the status-screen PlaceString label
strings (TypesIDNoOTText, StatusText, OKText, StatsText, StatusScreenExpText) were
first hand-encoded as raw charmap hex in the .asm — WRONG per the two-tier rule.
They are now GENERATED: tools/gen_status_strings.py charmap-encodes them via
gb_text.encode (unicode_converter submodule) → assets/status_strings.inc (DO NOT
EDIT header), %included by status_screen.asm and wired into the Makefile `assets`
target (mirrors gen_menu_strings.py). Generated bytes are byte-identical to the
hand-encoded ones (verified). The OTPointers/NamePointers2 tables stay hand-written
in the .asm — they are port-specific WRAM-offset pointer tables (code, like a jump
table), not generatable data. The constants added to gb_constants.inc are correct
as hand edits (that include is the hand-maintained mirror of pret constants/, not
generated). NOTE for the worktree: gen_status_strings needs the unicode_converter
submodule (seeded from the primary clone); the generated .inc is committed/seeded.

CROSS-SUBSYSTEM FIX (user-approved): `home/print_num.asm` `PrintNumber` read
LITTLE-endian; pret reads BIG-endian (first byte at DE → most-significant slot of
hNumToPrint). The port had been big-endian everywhere it mattered (CalcStats stores
hi-then-lo; battle_hud/battle_menu roll their own big-endian print_num2/3), and
`PrintNumber` was the lone little-endian outlier — harmless only because every prior
caller passed 1-byte values. The status screen is the first multi-byte caller, so it
surfaced. Fixed `PrintNumber` to big-endian (also fixes latent multi-byte `TX_NUM`).
All existing 1-byte callers unaffected; full `make` + `make check` green. Documented
in CLAUDE.md (new "Data Endianness", "Preserve Flags", "Preserve pret Labels"
sections, all added this session at the user's request).

CalcExpToLevelUp REVIEW (user flagged "likely bespoke and incorrect"): reviewed
against pret status_screen.asm — it is a FAITHFUL, correct translation (the
big-endian sub→sbb borrow chain works because inc/dec on the pointer preserve CF;
mov between the halves doesn't touch flags). Kept verbatim (moved into the new file).
No rewrite needed.

DONE (src/engine/pokemon/status_screen.asm, in POKEMON_SRCS):
- PROJECTION `scoord(x,y) = W_TILEMAP + (y+3)*40 + (x+10)` — centers the 20x18 GB
  screen in the 40x25 widescreen BG canvas, matching battle_menu's PrintStatsBox
  precedent. text_row_stride set to 40 in the StatusScreen prologue (like init_battle;
  battle_menu restores it to 20). Flat-canvas render setup mirrors init_battle
  (view-ptr=0, SCX/SCY=0, hide_window, LoadHpBarAndStatusTilePatterns +
  LoadHudTilePatterns — the latter supplies the $76/$77/$78/$6f line-box tiles,
  replacing pret's 4x CopyVideoDataDouble(BattleHudTiles/PTile)).
- StatusScreen (page 1): LoadMonData + box/daycare stat recalc, name/level (":L"+
  PrintLevel), HP bar (DrawHP, faithful GetHPBarLength+DrawHPBar incl. the cosmetic
  >=256 ÷4 bug under BUG_FIX_LEVEL<2), STATUS/OK (PrintStatusCondition), types
  (PrintMonType via WideTypeNames + single-type EraseType2Text), pokédex # (inline
  IndexToPokedex table lookup), OT name + ID# + № marker, stats box (local
  StatusScreen_StatsBox = pret PrintStatsBox d=0 branch — kept separate from
  battle_menu's d!=0 PrintStatsBox, which reads the party struct because battle
  stubs LoadMonData). DrawLineBox + .GetStringPointer ported faithfully.
- Palette calls (GetHealthBarColor/RunPaletteCommand SET_PAL_STATUS_SCREEN) kept
  faithful but inert (RunPaletteCommand is a Phase-5 no-op). rAUDVOL/hTileAnimations/
  cry = TODO-HW (audio/tile-anim HAL, Phase 3).
- Includes: gb_memmap += wLoadedMonMoves/OTID/Attack/Defense/Speed/Special,
  wStatusScreenHPBarColor/CurrentPP (all sym-verified); gb_constants +=
  STATUS_SCREEN_STATS_BOX, SET_PAL_STATUS_SCREEN, BIT_NO_AUDIO_FADE_OUT, LEADING_ZEROES.
- DEBUG harness: `make DEBUG_STATUS=1` seeds STARTER_PIKACHU (slot 3), opens page 1,
  dumps FRAME.BIN (RunStatusScreenTest in debug_dump.asm + StatusScreen in-routine
  hook + overworld.asm gate + Makefile). LIVE RENDER CONFIRMS: PIKACHU / :L5 /
  HP:19/19 bar / STATUS/OK / 025 / TYPE1 ELECTRIC (TYPE2 erased) / ID 00000 / OT
  NINTEN / ATTACK 11 DEFENSE 8 SPEED 15 SPECIAL 10 — stats CORRECT (not byte-swapped),
  proving the big-endian fix end-to-end.

STILL TODO (Stage 4 remainder):
1. Mon front pic (TODO-PIC in StatusScreen at hlcoord 1,0): the port's
   LoadFlippedFrontSpriteByMonIndex loads VRAM only (EDX=dest) — needs the 7x7
   tilemap-block placement wired for the non-battle canvas + live check.
2. Cry (TODO-HW audio, Phase 3).
3. Caller wiring: party-menu STATS branch (party_menu.asm:324 deferred no-op →
   StatusScreen[+StatusScreen2]) — that file is the concurrent menus agent's;
   coordinate. Battle level-up already calls battle_menu's PrintStatsBox (d!=0),
   which is the correct split, so no change there for the stats box; the
   summary-screen STATS entry is the outstanding wire.
   (Harness for both pages: `make DEBUG_STATUS=1` / `DEBUG_STATUS_PAGE2=1`.)
================================================================================ -->
