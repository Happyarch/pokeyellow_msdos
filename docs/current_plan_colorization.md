...
- [ ] Ride-along: preview/debug tooling (PAL.BIN + render_frame.py lockstep, with R1)
...
- Sprites: `render_sprites` already bands by palette (`4+color`/`8+color` off `OAM_PAL1`). Widen to `32 + (obj_slot << 2) + color` using `OAM_PAL1` + `OAM_HIGH_PALS` (already carried into shadow OAM by `PrepareOAMData`, currently dead) → 4 OBJ slots now, 8 possible with one more attr bit later.
...
- [ ] `boot/video.asm commit_palette`: 64 entries, per-slot PAL deref + DMG-register remap, gate on BGP/OBP change OR `g_pal_dirty`; boot defaults = visual parity (above).
- [ ] `src/ppu/ppu.asm`: `rebuild_tile_cache` OR-in `tile_pal[id]<<2` (or pre-banded LUT variant if perf 4a landed); `render_sprites` obj-slot widening + the `test …,3` BG-priority fix; `render_bg` untouched.
...
- [ ] HP-bar tile duplication at battle HUD load (`src/engine/battle/battle_hud.asm`).
- [ ] `SlideBattlePicsIn` (`src/home/pics.asm`): replace the `dmg_palette` shade-3 poke with faithful SET_PAL_BATTLE_BLACK slot swap.
- [ ] Real `UpdateCGBPal_BGP/OBP0/OBP1` bodies (= arm `g_pal_dirty`; fades already flow through commit's remap) — fills the four `TODO-HW` sites in `src/home/fade.asm`, retires the `UpdateCGBPal_OBP1` ret-stub in `overworld_stubs.asm`.
- [ ] faithdiff justification: SGB packet bit-banging realized as data + HAL dispatch (not ported verbatim).

## Stage R2 — Repaint override consumption

- [ ] At pic-load (`src/home/pics.asm`, post-interlace): if species has a repaint record, blit override 2bpp over the VRAM pic tiles, write its per-pic-tile grid into `tile_pal[picbase..]`, load extra palettes into free BG slots, arm both dirty flags. Default `UncompressSpriteData` path untouched (glitch fidelity).

## Stage R3 — Overworld / menus (later; rides the perf-rebased renderer)

- [ ] `SetPal_Overworld` (map-id+1 → PAL_ROUTE.., cave/tower special cases), remaining `SetPal_*` screens, per-tileset tile-ID tables, NPC OBJ palettes.

## Preview/debug tooling (rides R1)

- [ ] Debug dump also writes `PAL.BIN` (live 64-entry DAC + `tile_pal` + slot tables).
- [ ] `tools/render_frame.py` + `gfx_core/tiles.py`/`surface.py`: consume `PAL.BIN` when present for banded values 0–63; fallback debug palette updated for the new sprite band (4–11 → 32–63). Keep the two in lockstep (documented invariant).
