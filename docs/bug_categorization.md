...
|---|---|---|---|---|---|
| Badge Stat Boost Glitch | BUG(general) | critical — 999-cap stat corruption over a battle | Yes | `src/engine/battle/effects.asm:.applyBadgeBoostsAndStatusPenalties` | Yes |
...
| Bide vs Fly/Dig | BUG(general) | critical (ambiguous default) | **No** | pret `engine/battle/effects.asm` — only Bide's SETUP half is ported (`effects.asm:BideEffect`); the damage-accumulation/release-on-expiry half (where this bug lives) doesn't exist anywhere in `dos_port/src` | Pending port |
...
| Hyper Beam + Sleep | BUG(general) | cosmetic — bypasses hit-tests but pret's own comment flags it; no corruption | Yes | `src/engine/battle/effects.asm:SleepEffect` | Yes (pre-existing, incl. a `BUG_FIX_LEVEL>=2` real fix block) |
...
| ws# #m# (Yellow ACE glitch item) | GLITCH (YES ACE) | GLITCH | **Unmeasured** | The old blocker claim is retired: `UseItem_` and `ItemUsePtrTable` are linked, and `UseItem_` indexes the table without an explicit bounds check. Reachability and the exact out-of-range target for this glitch item have not been executed, so this row cannot yet claim either faithful ACE behavior or a port crash. | Pending runtime classification |
...
| Item slot $FF | BUG(critical), ACE: Indirect | critical | **Unmeasured** | The generic `UseItem_` → `ItemUsePtrTable` dispatch now exists and has no explicit bounds check. The slot-$FF acquisition/use path and the resulting out-of-range flat pointer have not been executed, so classify it from runtime evidence rather than the retired "no dispatcher" claim. | Pending runtime classification |
...
| Nidorino Cry Mismatch | BUG(general) | cosmetic — wrong cry SFX only | **No** | Requires the title-screen/intro rival-Nidorino demo battle, and no demo battle sequence is ported. (The old rider — that the title screen itself "does NOT render fully correctly" — was a stale CLAUDE.md entry, retired 2026-08-16; it never bore on this row, which turns on the missing demo battle alone.) | Pending port |
| Battle Draw Victory Fanfare | BUG(general) | cosmetic — wrong music cue only | **No** | Requires mutual-faint ("draw") detection/resolution on a Self-Destruct/Explosion KO; no such handling found in `faint_enemy.asm`/`faint_leaves.asm`/`effects.asm:ExplodeEffect` | Pending port |
