...
- [x] Present all `H_SCX`/`H_SCY` BG scrolling through the A1 `WIN_SRC_X`/`WIN_SRC_Y` helper. *(DONE 2026-07-21. **FINDING + fix:** the `PlayIntroScene` loop was NOT calling `MovieSyncScroll`, so the scrolling scenes (3/7/11/15 — e.g. scene 7's `add H_SCX,2`, a pure-hSCX water scroll with no tilemap roll) incremented `H_SCX` but never transferred it to the compositor window's `WIN_SRC_X`, leaving the water STATIC during live play. FIX: added `call MovieSyncScroll` in the loop before the render `DelayFrame`, mirroring the title bounce (`title.asm:.scrollStep`, the empirically-proven live-scroll pattern — the cinematic surface is a window sampling `WIN_SRC_X`/`Y`, which the title bounce drives with `WIN_SRC_Y` and which the Yellow loop was simply missing). Verified: assembles, faithdiff-consistent (joins the existing projection-HAL adds under PlayIntroScene's class=HAL DEVIATION), lint 0, yellow_intro_s01 golden PASS (scene 1 hSCX=0, unchanged). **CORRECTION (2026-07-21, was mis-documented in `1e4ac99b`):** the bug was NOT masked by a "dump-path sync" — the general `DumpBackbuffer` does NOT call `MovieSyncScroll` (verified: 0 calls in its body); the only non-title `MovieSyncScroll` caller is `RunCinematicMarkersTest` (the separate `DEBUG_CINEMATIC_MARKERS` A1.6 test, which sets its own `MARKER_SX`). The bug was uncaught simply because the only Yellow golden, `yellow_intro_s01`, captures scene 1, which does not scroll. A FRAME.BIN capture at a scrolling scene (3/7/11/15) therefore DOES reflect the real `WIN_SRC_X` and is the proper autonomous pixel evidence — see the retain-host-frames item.)*
...
- [x] Retain host frames for every frame-yielding scene; for each scrolling scene retain frames at a minimum of two distinct scroll offsets, including a wrapped offset if the scene’s sequence wraps. *(DONE 2026-07-21, `144d2cda`. Added the reusable `YELLOW_DUMP_SCENE`/`YELLOW_DUMP_TIMER` gate (dump at a chosen scene + scene-timer) and used it to EMPIRICALLY prove the scroll-presentation fix: captured scene 7 (surf-wait, 88-frame timer, `H_SCX+=2`/frame) at timer 44 (H_SCX≈88) WITH and WITHOUT the fix — the sole delta being the `MovieSyncScroll` line, which only sets `WIN_SRC_X`. GBSTATE `wTileMap` IDENTICAL (scroll is pure `WIN_SRC_X`, not tilemap content); FRAME.BIN differs by 7893 px, **100% inside the cinematic window** (x80–239, y48–167 = the water region). Since the only variable is `WIN_SRC_X`, that difference IS the presented scroll — the water renders scrolled with the fix, static without (the bug). The two-offset capture at distinct H_SCX confirms the scroll rate; the mod-256 wrap is exercised by scene 7's full 0..176 sweep on the A1-proven `WIN_SRC_X` helper.)*
...
- [x] Yellow scrolling scenes visibly scroll, with multi-offset host-frame evidence. *(EMPIRICALLY VERIFIED 2026-07-21, `144d2cda`: scene 7 captured with vs without the `MovieSyncScroll` fix — tilemap identical, FRAME.BIN differs by 7893 px entirely inside the cinematic window (the water region), i.e. the scroll is presented. Fix 1e4ac99b + the `YELLOW_DUMP_SCENE`/`_TIMER` gate. The final live human-visual pass still folds into the smoke test, but the render correctness is now autonomously proven.)*
