...
- [ ] Retain host frames for every frame-yielding scene; for each scrolling scene retain frames at a minimum of two distinct scroll offsets, including a wrapped offset if the scene’s sequence wraps. *(AUTONOMOUSLY CAPTURABLE — not human-gated (correcting the earlier mis-statement): the general `DumpBackbuffer` does not sync scroll, so a scene-3/7/11/15 FRAME.BIN reflects the real `WIN_SRC_X`. Capture two frames of a scrolling scene at distinct `H_SCX` (scene 7 walks `H_SCX` +2/frame over its 40-frame timer → 0..80, crossing tile boundaries and wrapping) and diff the water band, à la `check_title_timing.py`. This is a bounded pixel-harness task (a scene-triggered dump gate + a horizontal-displacement measure); the scroll-presentation fix 1e4ac99b makes the render correct, this item is the pixel proof of it. Not yet built.)*
...
- [ ] Yellow scrolling scenes visibly scroll, with multi-offset host-frame evidence. *(the fine-scroll presentation is fixed (1e4ac99b) and the mechanism is A1-proven (title bounce uses the same WIN_SRC window sync). AUTONOMOUSLY CAPTURABLE — correcting an earlier mis-statement that this was masked by the dump path: `DumpBackbuffer` does not sync scroll (only the separate `DEBUG_CINEMATIC_MARKERS` test does), so a scene-3/7/11/15 FRAME.BIN reflects the real WIN_SRC_X. The multi-offset pixel capture is a bounded, not-yet-built harness task, not a human gate.)*
