...
- [x] Present all `H_SCX`/`H_SCY` BG scrolling through the A1 `WIN_SRC_X`/`WIN_SRC_Y` helper. *(DONE 2026-07-21. **FINDING + fix:** the `PlayIntroScene` loop was NOT calling `MovieSyncScroll`, so the scrolling scenes (3/7/11/15 — e.g. scene 7's `add H_SCX,2`, a pure-hSCX water scroll with no tilemap roll) incremented `H_SCX` but never transferred it to the compositor window's `WIN_SRC_X`. The water was therefore STATIC during live play; every GBSTATE/FRAME capture nonetheless looked correct because the dump path (`debug_dump.asm` `RunDump`) calls `MovieSyncScroll` before dumping — so no capture-based test (golden or pixel) could ever detect it. Confirmed by tracing: `MovieSyncScroll` had exactly two live callers, both the title bounce (`title.asm:.scrollStep`) and the dump path; the Yellow loop had none, and `MovieMirrorSurface` only mirrors tilemap content, not scroll. FIX: added `call MovieSyncScroll` in the loop before the render `DelayFrame`, mirroring the title bounce (the empirically-working live-scroll pattern). Verified: assembles, faithdiff-consistent (joins the existing projection-HAL adds under PlayIntroScene's class=HAL DEVIATION), lint 0, yellow_intro_s01 golden PASS (scene 1 hSCX=0, unchanged). Live-play visual confirmation of the water scroll folds into the pending human smoke test — captures cannot confirm it due to the dump-path-sync masking.)*
