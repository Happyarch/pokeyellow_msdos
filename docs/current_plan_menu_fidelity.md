...
### M-114 (row 21, FIXED) — pret's `GymLeaderFaceAndBadgeTileGraphics` was emitted under a port-invented name

`engine/menus/draw_badges.asm` has two top-level labels, not one: `DrawBadges` and
the data label `GymLeaderFaceAndBadgeTileGraphics:` (`INCBIN
"gfx/trainer_card/badges.2bpp"`). The port had the bytes — `tools/gen_badge_tiles.py`
emits them into `assets/badge_tiles.inc` — but under the invented symbol
`badge_face_tiles`, so translation.db read the pret label as **`missing`** even
though the data was present and loaded. That is a preserve-pret-labels violation
(the rule is hard, and it covers *data* labels, not just routines), and it is a
worked example of why "missing" in the DB is a hypothesis to check rather than an
absence to trust.

Fixed by making the generator emit pret's label; `draw_badges.asm` declares it
`global` (the include-defined-global path `update_label_db` already supports) and
`LoadBadgeTiles` sources from it. No pixel or address changed. The label now reads
`translated`. Note that `faithdiff GymLeaderFaceAndBadgeTileGraphics` reports
`body extraction failed (pret=ok, port=MISSING)` — a **third faithdiff blind spot**:
it cannot extract a body for a port label whose definition lives in a generated
`assets/*.inc` rather than in the `.asm` text. There is nothing to diff for an
INCBIN blob; do not chase it.

### M-115 (row 21, FIXED) — four dead `%ifndef` guards, and a comment describing a promotion that had already happened

`draw_badges.asm` carried local `%ifndef wBadgeNumberTile / … equ 0xCD3D / %endif`
blocks for the four badge-scratch WRAM symbols, with a comment: "Guarded so root can
promote them into gb_memmap.inc without a redefinition clash." Both halves were
false. (1) `gb_memmap.inc:1714-1717` **already** defines all four (sym-pinned, with
its own note about the promotion) — the file `%include`s it, so the promotion was
long done. (2) The guard cannot work anyway: NASM's `%ifndef` tests single-line
macros, not `equ` symbols, so the block always expanded and simply re-`equ`d the
