# AGENTS.md — Pokemon Yellow DOS Port

Project context for agent sessions. Read this at the start of every session.
...
If your session was not pre-registered by its host, open the project first
(context_open, then root_register); an already-registered session skips that
handshake. Subagents: a small number, for exploration only — the ROOT session
writes the plan.
...
The deep reference lives in **agent project skills** so it only loads into
context when needed. Loading the matching skill through the active agent's skill
mechanism is required before planning, editing, reviewing, or verifying work in
that area.
...
Do not rely on memory, AGENTS.md, TODOs, or plan files as substitutes for the
skill.
...
Exception: if the agent is running inside a self-contained web container / VM
where it owns the environment, installs may proceed without prompting.
