...
| `audio/audition.py` | Host-side music and SFX audition & A/B testing across OPL3 (native 48 kHz FM via NukedOPL), MT-32 (MUNT), General MIDI (FluidSynth), and authentic Game Boy hardware (Blargg's `Basic_Gb_Apu`). Features `--init` instant-startup audio precaching, live hot-reload, position-locked `[Tab]` revision A/B toggle, `[G]` Game Boy APU reference toggle (with non-destructive enhancement suppression) across all targets, dedicated SFX auditioning mode (`--list-sfx`, `[Tab]` Tuned OPL3 vs Raw Default, `[X]` auto-alternate), solo/mute, and persistent `.revisions/` | `build-and-debug` |
