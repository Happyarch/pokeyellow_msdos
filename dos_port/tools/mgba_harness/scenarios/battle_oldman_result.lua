...
-- battle_oldman_result — the RESULT/EXIT half of the per-battle-type box, for the
-- OLD_MAN tutorial. battle_oldman (id 69) stops at the rename inside the menu;
-- this one follows the same battle to its end.
--
...
scenario.run(function()
	...
	for _ = 1, 400 do
		scenario.exec(function()
			...
			dump.write("battle_oldman_result", dump.standard_regions(sym), {
				frame = scenario.frame(),
				description = "the OLD_MAN tutorial's capture tail: ItemUseBall took " ..
	...
	assert(dumped, "battle_oldman_result: wBattleResult never reached 2 — the " ..
