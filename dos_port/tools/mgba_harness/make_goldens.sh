...
#
# Ground-truth inputs come from the in-tree pret build (the branch tree builds
# the ROM itself — root `make yellow`):
#   $PRET_GOLDEN_DIR (default: the repo root) must hold pokeyellow.gbc +
#   pokeyellow.sym from `make yellow`. Override it to point at an isolated
#   pret checkout if you need one.
# The ROM is sha1-checked against roms.sha1 before every run — goldens are
# only ever generated from a verified ROM, wherever it was built.
#
...
for f in "$ROM" "$SYM"; do
    [ -f "$f" ] || { echo "ERROR: $f missing — build the golden ROM:" >&2
        echo "  make -C '$REPO_ROOT' yellow" >&2
        exit 1; }
done
