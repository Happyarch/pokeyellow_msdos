...
def main() -> int:
    ...
    # --- field_move_text.inc (OW-4.4 far streams, flattened) -----------------
    cm = gen_battle_text.load_charmap()
    mem = gen_battle_text.load_memmap()
    far = gen_battle_text.collect_far(cm, mem)
    fout = [
        "; field_move_text.inc — generated by tools/gen_overworld_strings.py. DO NOT EDIT BY HAND.",
        "; Field-move message FAR text streams (Strength/Surf/boulder), pret text_far",
        "; indirection flattened via gen_battle_text.collect_far (pret data/text/text_8.asm).",
        "; %included by src/engine/overworld/field_move_messages.asm.",
        "; section .data so labels never land in an orphaned section (see link.ld).",
        "",
        "section .data",
        "",
    ]
    for label in FIELD_MOVE_FAR:
        if label not in far:
            sys.stderr.write(f"gen_overworld_strings: missing far label {label}\n")
            return 1
        fout.append(fmt_bytes(label, far[label]))
    fout.append("")
    fdst = ASSETS / "field_move_text.inc"
    fdst.write_text("\n".join(fout))
    print(f"wrote {fdst} ({len(FIELD_MOVE_FAR)} far labels)")
    return 0
