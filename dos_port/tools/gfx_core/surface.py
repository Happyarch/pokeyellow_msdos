...
def blit_code(img: Image.Image, code: int, col: int, row: int) -> None:
    """Blit a charmap-coded font/border tile (the old render._blit_tile)."""
    blit_gb_tile(img, ft.tile_for_code(code), col, row)


def draw_box_border(img: Image.Image, col: int, row: int, w: int,
                    h: int) -> None:
    """TextBoxBorder-identical frame: w x h OUTER tiles (w,h >= 3)."""
    blit_code(img, ft.BOX_TL, col, row)
    blit_code(img, ft.BOX_TR, col + w - 1, row)
    blit_code(img, ft.BOX_BL, col, row + h - 1)
    blit_code(img, ft.BOX_BR, col + w - 1, row + h - 1)
    for x in range(1, w - 1):
        blit_code(img, ft.BOX_H, col + x, row)
        blit_code(img, ft.BOX_H, col + x, row + h - 1)
    for y in range(1, h - 1):
        blit_code(img, ft.BOX_V, col, row + y)
        blit_code(img, ft.BOX_V, col + w - 1, row + y)
        for x in range(1, w - 1):
            blit_code(img, ft.TILE_SPC, col + x, row + y)


def draw_label(img: Image.Image, s: str, col: int, row: int) -> None:
    """Charmap text; pret `next` = newline advancing 2 rows (menu spacing)."""
    r = row
    for line in s.split("\n"):
        for i, code in enumerate(ft.encode_label(line)):
            blit_code(img, code, col + i, r)
        r += 2
