...
def generate_map(map_id: int, const: str, label: str, charmap: list,
    ...
    script_externs = [(sym, sent) for _, _, sym, sent in npc_entries if sym]
    lines = [
    ...
    for sym, sent in script_externs:
        kind = ("hand-written text_asm script" if sent == SCRIPT_SENTINEL
                else "trainer header -> TrainerTalkHook")
