...
# Parameterised-hook entry form (map-script fidelity plan, Stage 2). pret's trainer
# talk hooks are all the same three instructions with one immediate changed:
#     <Map><Trainer>Text: text_asm / ld hl, <Map>TrainerHeaderN
#                         call TalkToTrainer / jp TextScriptEnd
# so instead of one hand-written port routine per trainer, the entry carries the
# TRAINER HEADER POINTER and the dispatcher calls the shared TrainerTalkHook
# (src/scripts/trainer_map_script.asm). The (map, text id) -> header mapping thus
...
def generate_map(map_id: int, const: str, label: str, charmap: list,
    ...
    if parsed is None:
        return None
    _border, _warps, signs, sprites = parsed
    npc_count = len(sprites)
    # A map with no NPCs but with signs STILL needs a table: its signs' text ids index
    # it. Bailing out here left MapTextTablePointers[map] = 0, so DisplaySignText died
    # at its null-table check and the sign said nothing — the second half of F-10, and
    # the reason the 1-based-id fix alone does not make those 7 maps' signs work.
    if npc_count == 0 and not signs:
        return None
    ...
    for i in range(npc_count_eff):
        ...
        if script_label:
            npc_entries.append((nasm_label, None, script_label, SCRIPT_SENTINEL))
            continue

        # Parameterised trainer talk hook — only for a map whose script layer is
        # live (gen_map_script_tables.WIRED_MAPS). The entry carries the trainer
        # HEADER pointer; the dispatcher calls the shared TrainerTalkHook.
        if local_label and const in mst.WIRED_MAPS and scripts_path.exists():
            header = _resolve_trainer_talk_hook(scripts_path, local_label)
            if header:
        ...
        if is_trainer:
            ...
            if local_label and scripts_path.exists():
                ...
                if far and far in text_db:
                    data = text_db[far]
        elif is_item or local_label == "PickUpItemText":
            npc_entries.append((nasm_label, None, "PickUpItemText", SCRIPT_SENTINEL))
            continue
        else:
            data = None
            if local_label and scripts_path.exists():
        ...
        npc_entries.append((nasm_label, data, None, None))

    # Emit NASM include file
    out_path = DIALOGS_DIR / f"{map_snake}_dialogs.inc"
    seen_externs = set()
    script_externs = []
    for _, _, sym, sent in npc_entries:
        if sym and sym not in seen_externs:
            seen_externs.add(sym)
            script_externs.append((sym, sent))
    lines = [
        f"; {out_path.name} — generated by tools/generators/gen_npc_dialogs.py. DO NOT EDIT BY HAND.",
        f"; Map: {const} (id=0x{map_id:02X}), {len(npc_entries)} text slot(s) (NPC + sign).",
        f"; Row k holds pret text id k+1 (ids are 1-based; the lookup subtracts one).",
        f"; Source: text/{label}.asm + scripts/{label}.asm",
        f";",
        f"; section .data so labels never land in an orphaned section.",
        f"",
    ]
    for sym, sent in script_externs:
        if sym == "PickUpItemText":
    ...
    if script_externs:
        lines.append("")
    lines += ["section .data", ""]
    for nasm_label, data, script_label, _sentinel in npc_entries:
        if script_label:
            continue   # SCRIPT / trainer-talk slots reference an extern, no stream
        lines.append(f"{nasm_label}:")
        lines.append(_bytes_to_nasm(data))
        lines.append(f"{nasm_label}_end:")
        lines.append("")
    lines.append(f"{table_label}:")
    for nasm_label, _data, script_label, sentinel in npc_entries:
...
def generate_all() -> None:
    ...
    lines = [
        "; all_dialogs.inc — generated by tools/generators/gen_npc_dialogs.py. DO NOT EDIT BY HAND.",
