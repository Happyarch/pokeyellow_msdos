...
# section the port hosts at its mirror engine/menus/start_sub_menus.asm). Same (label, [parts])
...
# pret's ContinueText ends in <NEXT> with NO terminator and FALLS THROUGH into
# NewGameText, so the save-present menu prints "CONTINUE / NEW GAME / OPTION" from one
# PlaceString — the two labels must stay adjacent and in this order in the emitted .inc.
# All three were hand-encoded charmap bytes in the port until menu-fidelity row 18
# (M-94); the generated bytes were byte-compared against the old literals and are
# identical.
MAIN_MENU_FAR = [
    "_NotEnoughMemoryText",         # "Not enough Yellow / Version memory."
]

MAIN_MENU = [
    ("ContinueText",                # pret: db "CONTINUE" next "" (falls through)
     ["CONTINUE", NEXT]),
    ("NewGameText",                 # pret: db "NEW GAME" next "OPTION@"
     ["NEW GAME", NEXT, "OPTION", TERM]),
    ("SaveScreenInfoText",          # pret: db "PLAYER" next "BADGES    " next "#DEX    " next "TIME@"
     ["PLAYER", NEXT, "BADGES    ", NEXT, "#DEX    ", NEXT, "TIME", TERM]),
]


# The SAVE / LOAD side of pret engine/menus/save.asm (menu-fidelity row 19 part 1):
# its five text_far message streams (pret data/text/text_4.asm). The port had NO pret
# text labels for these at all — it hand-encoded each LINE as its own charmap `db` run
...
# The CABLE CLUB link menus (pret engine/menus/link_menu.asm — the port splits it
# into link_menu.asm + link_cups.asm). Twenty text_far message streams (bodies in
# data/text/text_3.asm) plus the six `db` strings pret writes inline in the code file.
#
# Until menu-fidelity row 20 part 1, NONE of this was data: every line of every
# message was a separate HAND-ENCODED charmap `db` run, printed by a private
# lm_msg_* engine, and the twenty pret text_far LABELS had been redefined as print
# ROUTINES (`call ColosseumMewText`) — so the three name-splicing streams
# (_Colosseum{Height,Weight,Evolved}Text are `text_ram wNameBuffer` texts) were
# imitated with an extra place_flat_str call, and every `prompt`/`done` was
# re-implemented by hand (M-109).
LINK_FAR = [
    "_Colosseum3MonsText",          # PokeCup/PikaCup/PetitCup: not 3 mons
    "_ColosseumMewText",
    ...
    "_ColosseumHeightText",         # text_ram wNameBuffer + " is over / 6’8” tall!"
    "_ColosseumWeightText",         # text_ram wNameBuffer + " weighs / over 44 pounds!"
    "_ColosseumEvolvedText",        # text_ram wNameBuffer + " is an / evolved #MON!"
    "_ColosseumIneligibleText",     # the REMOTE player failed his cup check
    "_ColosseumWhereToText",        # LinkMenu's prompt (`done` — the box stays up)
...
# LIGATURE ($BE, ONE glyph in "can't") and the <PKMN> ($4A) text command, neither of
# which gb_text.encode's single-char pass can produce (the APOS_S note above).
LINK_STRINGS = [
    ("Text_f56f4", ["LVs of 3<PKMN>:50-55", NEXT, "Sum of LVs:155 MAX", NEXT,
                    "MEW can't attend.", TERM]),
    ("Text_f5728", ["LVs of 3<PKMN>:15-20", NEXT, "Sum of LVs:50 MAX", NEXT,
                    "MEW can't attend.", TERM]),
    ("Text_f575b", ["3 Basic <PKMN>.LV25-30", NEXT, "Sum of LVs:80 MAX", NEXT,
                    "6’8” and 44lb MAX", TERM]),
    ("Text_f5791", ["View", NEXT, "Rules", TERM]),
    ("Text_f579c", ["# Cup", NEXT, "Pika Cup", NEXT, "Petit Cup", NEXT,
                    "CANCEL", TERM]),
...
# BILL's PC (pret engine/pokemon/bills_pc.asm): the storage-box UI's fourteen
# text_far message streams (bodies in data/text/text_3.asm — _MonWasStoredText
# splices the 3-byte wBoxNumString the deposit path composes at $CD3D with
# TX_RAM) plus the eleven `db` strings pret writes inline in the code file: the
# PC main-menu entry labels (DisplayPCMainMenu), the six-entry box-UI menu
# (BillsPCMenuText), the BOX No. readout, and the deposit/withdraw submenu.
# The db strings are encoded through PRET's charmap (greedy longest-match,
# encode_parts_pret): four of them carry the "'s" LIGATURE ($BD, ONE glyph in
# "SOMEONE's"/"BILL's"/"'s PC"/"PROF.OAK's") and two carry the <PKMN> text
# command ($4A) — both of which gb_text.encode's single-char pass gets wrong
# (see APOS_S above).
BILLS_PC_FAR = [
    "_SwitchOnText",                # BillsPC_ direct access: "Switch on!"
    "_WhatText",                    # pret-unreferenced; kept for mirror completeness
    "_DepositWhichMonText",         # pret-unreferenced; kept for mirror completeness
    "_MonWasStoredText",            # "<mon> was / stored in BOX <wBoxNumString>." (TX_RAM)
    "_CantDepositLastMonText",      # deposit with one party mon left
    "_BoxFullText",                 # deposit into a full current box
    "_MonIsTakenOutText",           # withdraw done
    "_NoMonText",                   # withdraw/release from an empty box
    "_CantTakeMonText",             # withdraw with a full party
    "_PikachuUnhappyText",          # release the starter Pikachu: refused
    "_ReleaseWhichMonText",         # pret-unreferenced; kept for mirror completeness
    "_OnceReleasedText",            # release confirm prompt (before YesNoChoice)
    "_MonWasReleasedText",          # release done
    "_SleepingPikachuText2",        # deposit the following starter Pikachu: refused
]

BILLS_PC_STRINGS = [
    # --- DisplayPCMainMenu's entry labels ---
    ("SomeonesPCText",   ["SOMEONE's PC", TERM]),    # pret: db "SOMEONE's PC@"
    ("BillsPCText",      ["BILL's PC", TERM]),       # pret: db "BILL's PC@"
    ("PlayersPCText",    ["'s PC", TERM]),           # drawn after wPlayerName
    ("OaksPCText",       ["PROF.OAK's PC", TERM]),   # pret: db "PROF.OAK's PC@"
    ("PKMNLeaguePCText", ["<PKMN>LEAGUE", TERM]),    # <PKMN> = $4A text command
    ("LogOffPCText",     ["LOG OFF", TERM]),
    # --- the box UI (BillsPCMenu / DisplayDepositWithdrawMenu) ---
    ("BillsPCMenuText",                              # pret: db/next x6 + "@"
     ["WITHDRAW <PKMN>", NEXT, "DEPOSIT <PKMN>", NEXT, "RELEASE <PKMN>", NEXT,
      "CHANGE BOX", NEXT, "PRINT BOX", NEXT, "SEE YA!", TERM]),
    ("BoxNoPCText",      ["BOX No.", TERM]),         # pret: db "BOX No.@"
    ("DepositPCText",    ["DEPOSIT", TERM]),
    ("WithdrawPCText",   ["WITHDRAW", TERM]),
    ("StatsCancelPCText",                            # pret: db "STATS" next "CANCEL@"
     ["STATS", NEXT, "CANCEL", TERM]),
]
...
def encode_parts_pret(parts, cm):
    """Same, but through pret's own charmap (greedy longest-match, gen_battle_text).

    Use this for any string carrying a ligature ("can't" → $BE) or a text COMMAND
    written as a token ("<PKMN>" → $4A); gb_text.encode matches one character at a
    time and silently emits a longer, wrong byte run for both.
    """
    out = []
    for p in parts:
        if isinstance(p, int):
            ...
            out.extend(gen_battle_text.encode(p, cm))
    return out
...
def main() -> int:
    out = [
        "; menu_strings.inc — generated by tools/generators/gen_menu_strings.py. DO NOT EDIT BY HAND.",
    ...
    fout = [
        "; menu_text.inc — generated by tools/generators/gen_menu_strings.py. DO NOT EDIT BY HAND.",
    ...
    cout = [
        "; pc_text.inc — generated by tools/generators/gen_menu_strings.py. DO NOT EDIT BY HAND.",
    ...
    ppout = [
        "; players_pc_text.inc — generated by tools/generators/gen_menu_strings.py. DO NOT EDIT BY HAND.",
    ...
    oout = [
        "; oaks_pc_text.inc — generated by tools/generators/gen_menu_strings.py. DO NOT EDIT BY HAND.",
    ...
    lout = [
        "; league_pc_text.inc — generated by tools/generators/gen_menu_strings.py. DO NOT EDIT BY HAND.",
    ...
    # --- the boot MAIN MENU: its three db strings + _NotEnoughMemoryText -------
    mout = [
        "; main_menu_text.inc — generated by tools/generators/gen_menu_strings.py. DO NOT EDIT BY HAND.",
        "; The boot main menu's strings (pret engine/menus/main_menu.asm): ContinueText /",
        "; NewGameText / SaveScreenInfoText, GB-charmap encoded via gb_text.encode, and",
        "; _NotEnoughMemoryText (pret data/text/text_3.asm, text_far body flattened via",
        "; gen_battle_text.collect_far). src/engine/menus/main_menu.asm %includes this and",
        "; keeps pret's wrapper label (NotEnoughMemoryText = text_far _NotEnoughMemoryText",
        "; + text_end). ContinueText deliberately has NO terminator: it falls through into",
        "; NewGameText, exactly as in pret — do not reorder these two labels.",
        "; section .data so the labels land in a loaded section (see link.ld).",
        "",
        "section .data",
        "",
    ]
    for label, parts in MAIN_MENU:
        b = encode_parts(parts)
        mout.append(f"{label}: db " + ", ".join(f"0x{x:02X}" for x in b))
    for label in MAIN_MENU_FAR:
        if label not in far:
            sys.stderr.write(f"gen_menu_strings: missing far label {label}\n")
            return 1
        rows = [
    ...
    # --- SAVE / LOAD message streams (save.asm) -------------------------------
    sout = [
        "; save_text.inc — generated by tools/generators/gen_menu_strings.py. DO NOT EDIT BY HAND.",
        "; The SAVE/LOAD message streams of pret engine/menus/save.asm (bodies from",
        "; data/text/text_4.asm, flattened via gen_battle_text.collect_far).",
        "; src/engine/menus/save.asm %includes this and keeps pret's wrapper labels",
        "; (SavingText = text_far _SavingText + text_end), so PrintText prints them —",
        "; including _GameSavedText's TX_RAM player-name splice — exactly as pret does.",
        "; section .data so the labels land in a loaded section (see link.ld).",
        "",
        "section .data",
        "",
    ]
    for label in SAVE_FAR:
        if label not in far:
            sys.stderr.write(f"gen_menu_strings: missing far label {label}\n")
            return 1
        rows = [
    ...
    # --- CABLE CLUB link menus: 20 far streams + 6 db strings (link_menu.asm) --
    kout = [
        "; link_text.inc — generated by tools/generators/gen_menu_strings.py. DO NOT EDIT BY HAND.",
        "; Every rendered string of pret engine/menus/link_menu.asm: its twenty text_far",
        "; message streams (bodies in data/text/text_3.asm, flattened via",
        "; gen_battle_text.collect_far — three of them splice the mon name with TX_RAM)",
        "; and the six `db` strings pret writes inline in the code file (the cup-rules",
        "; panels, the View/Rules and cup lists, and TradeCenterText).",
        "; src/engine/menus/link_menu.asm %includes this and keeps pret's own wrapper",
        "; labels (ColosseumMewText = text_far _ColosseumMewText + text_end), so PrintText",
        "; prints them — prompt/done and all — exactly as pret does.",
        "; The db strings are encoded through PRET's charmap (greedy longest-match): they",
        "; carry the \"'t\" ligature ($BE, one glyph) and the <PKMN> text command ($4A).",
        "; section .data so the labels land in a loaded section (see link.ld).",
        "",
        "section .data",
        "",
    ...
    for label, parts in LINK_STRINGS:
        b = encode_parts_pret(parts, cm)
        kout.append(f"{label}: db " + ", ".join(f"0x{x:02X}" for x in b))
    kout.append("")
    for label in LINK_FAR:
        if label not in far:
            sys.stderr.write(f"gen_menu_strings: missing far label {label}\n")
            return 1
        rows = [
    ...
    bout = [
        "; bills_pc_text.inc — generated by tools/generators/gen_menu_strings.py. DO NOT EDIT BY HAND.",
        "; Every rendered string of pret engine/pokemon/bills_pc.asm: the PC main-menu",
        "; entry labels (DisplayPCMainMenu), the box-UI menu/readout/submenu strings, and",
        "; the fourteen text_far message streams (bodies in data/text/text_3.asm,",
        "; flattened via gen_battle_text.collect_far — _MonWasStoredText splices the",
        "; wBoxNumString the deposit path composes with TX_RAM).",
        "; src/engine/pokemon/bills_pc.asm %includes this and keeps pret's own wrapper",
        "; labels (MonWasStoredText = text_far _MonWasStoredText + text_end), so",
        "; PrintText prints them — prompt/done and all — exactly as pret does.",
        "; The db strings are encoded through PRET's charmap (greedy longest-match):",
        "; four carry the \"'s\" ligature ($BD, one glyph) and two the <PKMN> command ($4A).",
        "; section .data so the labels land in a loaded section (see link.ld).",
        "",
        "section .data",
        "",
    ]
    bcm = gen_battle_text.load_charmap()
    for label, parts in BILLS_PC_STRINGS:
        b = encode_parts_pret(parts, bcm)
        bout.append(f"{label}: db " + ", ".join(f"0x{x:02X}" for x in b))
    bout.append("")
    for label in BILLS_PC_FAR:
        if label not in far:
            sys.stderr.write(f"gen_menu_strings: missing far label {label}\n")
            return 1
        rows = [
    ...
    tout = [
        "; trainer_card_strings.inc — generated by tools/generators/gen_menu_strings.py. DO NOT EDIT BY HAND.",
    ...
    pout = [
        "; party_menu_strings.inc — generated by tools/generators/gen_menu_strings.py. DO NOT EDIT BY HAND.",
    ...
    hout = [
        "; home_pokemon_strings.inc — generated by tools/generators/gen_menu_strings.py. DO NOT EDIT BY HAND.",
    ...
    oout = [
        "; options_strings.inc — generated by tools/generators/gen_menu_strings.py. DO NOT EDIT BY HAND.",
    ...
    nout = [
        "; naming_strings.inc — generated by tools/generators/gen_menu_strings.py. DO NOT EDIT BY HAND.",
    ...
    pout = [
        "; pokedex_strings.inc — generated by tools/generators/gen_menu_strings.py. DO NOT EDIT BY HAND.",
