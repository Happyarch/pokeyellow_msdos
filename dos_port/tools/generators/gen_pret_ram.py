...
def main():
    ...
    for line in sym.read_text(errors="ignore").splitlines():
        ...
        if not re.match(r"^[wh]([A-Z]|[0-9a-f]{2,4}$)", name):
            continue
        if not (0xC000 <= addr <= 0xFFFF):          # WRAM, OAM, HRAM (no echo mirror)
            continue
    ...
    lines = [
        ...
        "; DO NOT EDIT BY HAND.",
        ";",
        "; Every pret WRAM/HRAM label, at its PORT address — rgblink's pokeyellow.sym",
        "; address shifted by the prefix-sum expansion (_shift) for symbols below a",
        "; grown region — as %define (a preprocessor macro) rather than equ.",
