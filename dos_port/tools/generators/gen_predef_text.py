...
This data IS consumed as of 2026-08-02: src/home/predef_text.asm links, and the
...
OUT_IDS = ROOT / "dos_port" / "assets" / "predef_text_ids.inc"
...
def main() -> int:
    ...
    for label in labels:
        ...
        # The same byte count as an assembly-time constant, so the hand-written
        # TextPredefs table in src/data/predef_text_data.asm can build its
        # {dd stream, dd size} rows without either duplicating the number by hand or
        # inferring it from label adjacency.
    ...
    # The predef ids go in their OWN include, deliberately: assets/predef_text.inc
    ...
    ids = [
        "; predef_text_ids.inc — generated by tools/generators/gen_predef_text.py. DO NOT EDIT BY HAND.",
        "; The 68 TextPredefs ids, in pret order (data/text_predef_pointers.asm).",
        "; Constants only — no section, no globals — so any caller can include it.",
        "",
    ] + [f"{label}_id equ {i}" for i, label in enumerate(entries, 1)] + [""]
    OUT_IDS.write_text("\n".join(ids))
