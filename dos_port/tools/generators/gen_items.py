...
def main() -> int:
    ...
    # ONE OUTPUT PER PRET SOURCE FILE for every table that HAS a pret data/ label,
    # so each can be carried by a .asm at its mirrored path
    # (docs/current_plan_data_path_mirror.md). items.inc keeps only the tables with
    # no single pret data/ home of their own (marts, VitaminStats, UsableItems_*).
    def header(stem, *blurb):
        return [f"; {stem}.inc — generated by tools/generators/gen_items.py."
                " DO NOT EDIT BY HAND.", *(f"; {b}" for b in blurb), ""]

    names_out = header("item_names",
                       f"ItemNames (data/items/names.asm): {len(names)} "
                       "'@'-terminated charmap names (variable length).")
    names_out.append("ItemNames:")
    o = 0
    for nm in names:
        enc = encode(nm, charmap)
        rec = namebytes[o:o + len(enc) + 1]
        o += len(enc) + 1
        names_out.append("    db " + ", ".join(f"0x{b:02X}" for b in rec) + f'    ; {nm}')
    names_out += ["ItemNames_end:", ""]

    prices_out = header("item_prices",
                        f"ItemPrices (data/items/prices.asm): {len(prices)} x "
                        "3-byte BCD prices, item-id order.")
    prices_out.append("ItemPrices:")
    for i, p in enumerate(prices):
        rec = pricebytes[i * 3:i * 3 + 3]
        prices_out.append("    db " + ", ".join(f"0x{b:02X}" for b in rec)
                          + f"    ; item {i+1}: {p}")
    prices_out += ["ItemPrices_end:", ""]

    key_out = header("key_item_flags",
                     f"KeyItemFlags (data/items/key_items.asm): {len(keybits)}-bit "
                     "array (LSB-first), TRUE = key item",
                     "(untossable). Test bit (item_id - 1). HMs ($C4-$C8) are key "
                     "via a separate range check, not this table.")
    key_out += ["KeyItemFlags:",
                "    db " + ", ".join(f"0x{b:02X}" for b in keyflags),
                ""]

    tmprice_out = header("tm_prices",
                         f"TechnicalMachinePrices (data/items/tm_prices.asm): "
                         f"{len(tm_nybbles)} TM prices (thousands), packed",
                         "two nybbles per byte, high nybble first (GetMachinePrice "
                         "indexes this).")
    tmprice_out += ["TechnicalMachinePrices:",
                    "    db " + ", ".join(f"0x{b:02X}" for b in tm_pricebytes),
                    ""]

    tmhm_out = header("tmhm_moves",
                      f"TechnicalMachines (data/moves/tmhm_moves.asm): "
                      f"{len(tm_moves)} TM + {len(hm_moves)} HM move ids,",
                      "TMs first then HMs, 0xFF-terminated (CanLearnTM / TMToMove "
                      "index this).")
    tmhm_out += ["TechnicalMachines:",
                 "    db " + ", ".join(f"0x{b:02X}" for b in tmhm_bytes),
                 ""]

    out = [
        "; items.inc — generated by tools/generators/gen_items.py. DO NOT EDIT BY HAND.",
        "; Item tables with no single pret data/ file of their own. The per-file",
        "; tables live in item_names.inc / item_prices.inc / key_item_flags.inc /",
        "; tm_prices.inc / tmhm_moves.inc beside this one.",
        "",
    ]
    ...
    for stem, body in (("item_names", names_out), ("item_prices", prices_out),
                       ("key_item_flags", key_out), ("tm_prices", tmprice_out),
                       ("tmhm_moves", tmhm_out)):
        (ASSETS / f"{stem}.inc").write_text("\n".join(body))
    dst = ASSETS / "items.inc"
    dst.write_text("\n".join(out))
    print(f"wrote {dst} + 5 per-file .inc (ItemNames {len(names)} names / {len(namebytes)} bytes, "
