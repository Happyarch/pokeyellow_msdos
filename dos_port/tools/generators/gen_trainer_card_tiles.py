#!/usr/bin/env python3
"""Generate dos_port/assets/trainer_card_tiles.inc — the trainer-card (player
status screen) tile graphics that pret's DrawTrainerInfo loads into VRAM, from
pret's already-2bpp graphics.

Faithful to pret engine/menus/start_sub_menus.asm:DrawTrainerInfo, which loads:
  TrainerInfoTextBoxTileGraphics: INCBIN "gfx/trainer_card/trainer_info.2bpp"  (9 tiles)
      tiles 0-7 → vChars2 tile $77  (box-drawing corners/edges)
      tile  8   → vChars1 tile $57  (box background)
  BlankLeaderNames:               INCBIN "gfx/trainer_card/blank_leader_names.2bpp"
      → vChars2 tile $60, `ld bc, $17 tiles` = 23 tiles
  BadgeNumbersTileGraphics:       INCBIN "gfx/trainer_card/badge_numbers.2bpp"  (8 tiles)
      → vChars1 tile $58
  the colon glyph = TextBoxGraphics (gfx/font/font_extra.2bpp) tile 13
      → vChars1 tile $56  (drawn as tilemap id $D6)

CircleTile — the ○ in "○BADGES○" — has NO load of its own, and no referencer
anywhere in pret. It rides along on the BlankLeaderNames copy: blank_leader_names
.2bpp is 22 tiles, DrawTrainerInfo copies 23, and gfx/trainer_card.asm places
CircleTile IMMEDIATELY after it, so the 23rd tile read is the circle and it lands
at vChars2 $60 + 22 = $76 — precisely the tile id TrainerInfo_BadgesText prints.
That adjacency is load-bearing, not incidental: this file therefore emits the two
blobs CONTIGUOUSLY (tc_circle_tile directly after tc_blank_name_tiles, nothing
between) and exports TC_BLANK_NAME_COPY_COUNT = 23 as the count to copy, so the
port reproduces pret's single copy instead of inventing a second one. Asserted
below — if pret's asset sizes ever change, this fails loudly rather than silently
loading the wrong 23rd tile.

The gym-leader face + badge sheet (gfx/trainer_card/badges.2bpp) is NOT bundled
here — it is already carried by tools/generators/gen_badge_tiles.py (assets/badge_tiles.inc)
and loaded by draw_badges.asm:LoadBadgeTiles, exactly the pixels pret loads to
vChars2 tile $20. DrawTrainerInfo (start_sub_menus.asm) calls LoadBadgeTiles for those.

This is Tier-1 machine output: a deterministic passthrough of pret .2bpp, no
hand-authored information. Rerunning is idempotent.

Output (NASM, .data): one label per blob + a TILE-count equate each.

Run from the repo root:
  python3 dos_port/tools/generators/gen_trainer_card_tiles.py
"""
from pathlib import Path

ROOT = Path(__file__).resolve().parents[3]
TC = ROOT / "gfx" / "trainer_card"
FONT_EXTRA = ROOT / "gfx" / "font" / "font_extra.2bpp"
DST = ROOT / "dos_port" / "assets" / "trainer_card_tiles.inc"

TILE = 16  # bytes per 2bpp 8x8 tile
COLON_TILE_INDEX = 13  # TextBoxGraphics (font_extra.2bpp) tile 13 = the ':' glyph


def read_tiles(path: Path) -> bytes:
    data = path.read_bytes()
    if len(data) % TILE:
        raise SystemExit(f"{path}: {len(data)} bytes is not a whole number of tiles")
    return data


def emit(out, label, data):
    out.append(f"{label}:")
    for t in range(len(data) // TILE):
        row = data[t * TILE:(t + 1) * TILE]
        out.append("    db " + ", ".join(f"0x{b:02X}" for b in row))
    out.append("")


def main():
    trainer_info = read_tiles(TC / "trainer_info.2bpp")
    if len(trainer_info) // TILE != 9:
        raise SystemExit(f"trainer_info.2bpp: expected 9 tiles, got {len(trainer_info)//TILE}")
    box_tiles = trainer_info[:8 * TILE]          # tiles 0-7 → vChars2 $77
    bg_tile = trainer_info[8 * TILE:9 * TILE]    # tile  8   → vChars1 $57

    blank_names = read_tiles(TC / "blank_leader_names.2bpp")   # → vChars2 $60
    badge_numbers = read_tiles(TC / "badge_numbers.2bpp")      # → vChars1 $58
    circle = read_tiles(TC / "circle_tile.2bpp")               # rides the copy above → $76

    # pret's DrawTrainerInfo copies $17 tiles from BlankLeaderNames; the asset is
    # one tile short of that, and CircleTile is what the last tile actually reads.
    copy_count = len(blank_names) // TILE + len(circle) // TILE
    if copy_count != 0x17:
        raise SystemExit(
            f"blank_leader_names + circle_tile = {copy_count} tiles, but pret's "
            f"DrawTrainerInfo copies $17 (23). The ○ would land at the wrong VRAM "
            f"slot — re-derive the load against engine/menus/start_sub_menus.asm."
        )

    font_extra = read_tiles(FONT_EXTRA)
    off = COLON_TILE_INDEX * TILE
    colon = font_extra[off:off + TILE]                         # → vChars1 $56 (id $D6)
    if len(colon) != TILE:
        raise SystemExit("font_extra.2bpp: no tile 13 (colon)")

    out = []
    out.append("; trainer_card_tiles.inc — generated by dos_port/tools/generators/gen_trainer_card_tiles.py.")
    out.append("; DO NOT EDIT BY HAND.  Trainer-card (player status) tile graphics (2bpp),")
    out.append("; passed through from pret gfx/trainer_card/*.2bpp + gfx/font/font_extra.2bpp.")
    out.append(f"TC_TILE_BYTES         equ {TILE}")
    out.append(f"TC_BOX_TILE_COUNT     equ {len(box_tiles)//TILE}")
    out.append(f"TC_TRAINER_INFO_COUNT equ {len(trainer_info)//TILE}   ; whole trainer_info.2bpp (see below)")
    out.append(f"TC_BLANK_NAME_COUNT   equ {len(blank_names)//TILE}")
    out.append(f"TC_BLANK_NAME_COPY_COUNT equ {copy_count}   ; pret: ld bc, $17 tiles")
    out.append(f"TC_BADGE_NUM_COUNT    equ {len(badge_numbers)//TILE}")
    out.append("")
    out.append("global tc_box_tiles   ; cable_club.asm's LoadTrainerInfoTextBoxTiles reads it (below)")
    out.append("; tc_box_tiles and tc_bg_tile are CONTIGUOUS on purpose — together they are")
    out.append("; the whole 9-tile trainer_info.2bpp, and cable_club.asm's")
    out.append("; LoadTrainerInfoTextBoxTiles copies TC_TRAINER_INFO_COUNT = 9 tiles from")
    out.append("; tc_box_tiles to vChars2 $76 in ONE CopyVideoData, exactly as pret does")
    out.append("; ((End - Start) / TILE_SIZE = 9). Do not insert anything between them.")
    emit(out, "tc_box_tiles", box_tiles)
    emit(out, "tc_bg_tile", bg_tile)
    out.append("; tc_blank_name_tiles and tc_circle_tile are CONTIGUOUS on purpose —")
    out.append("; pret's gfx/trainer_card.asm lays CircleTile straight after")
    out.append("; BlankLeaderNames, and DrawTrainerInfo's $17-tile copy walks off the")
    out.append("; end of the names into it. Do not insert anything between them.")
    emit(out, "tc_blank_name_tiles", blank_names)
    emit(out, "tc_circle_tile", circle)
    emit(out, "tc_badge_number_tiles", badge_numbers)
    emit(out, "tc_colon_tile", colon)

    DST.parent.mkdir(parents=True, exist_ok=True)
    DST.write_text("\n".join(out) + "\n")
    print(f"wrote {DST}: box={len(box_tiles)//TILE} names={len(blank_names)//TILE} "
          f"numbers={len(badge_numbers)//TILE} +bg +colon +circle")


if __name__ == "__main__":
    main()
