...
disassembly so the byte layout matches the original ROM exactly:
  ...
  PokedexOrder:   pret data/pokemon/dex_order.asm — internal index
...
def parse_species(rel_path: str, types, moves, growth, tmnum) -> bytes:
    ...
    We parse positionally (robust) and zero the DOS-irrelevant sprite block.
    The TM/HM bitfield (Stage 6) is filled from the `tmhm` macro's move list.
    """
    rows = []
    for line in (ROOT / rel_path).read_text().splitlines():
    ...
    # Collect the tmhm macro's move list (it may span lines via `\`).
    tmhm_moves = []
    for i, row in enumerate(rows):
        if row.startswith("tmhm"):
            joined = row
            j = i
            while joined.rstrip().endswith("\\"):
                j += 1
                joined = joined.rstrip()[:-1] + " " + rows[j]
            body = joined[len("tmhm"):].strip()
            tmhm_moves = [t.strip() for t in body.split(",") if t.strip()]
            break
    ...
    rec += tmhm_bitfield(tmhm_moves, tmnum)                        # tm/hm bitfield (7 bytes)
    rec += bytes(1)                                                # padding
...
def build_tmnum() -> dict:
    """Map move name -> TM/HM number (TMs 1..50 in add_tm order, HMs 51..55 in
    add_hm order), from constants/item_constants.asm."""
    text = (ROOT / "constants/item_constants.asm").read_text().splitlines()
    tms, hms = [], []
    for line in text:
        s = line.split(";", 1)[0].strip()
    ...
    tmnum = {}
    for i, mv in enumerate(tms):           # TMNUM 1..NUM_TMS
        tmnum[mv] = i + 1
    for i, mv in enumerate(hms):           # TMNUM NUM_TMS+1 ..
        tmnum[mv] = len(tms) + i + 1
    return tmnum


def tmhm_bitfield(moves_list, tmnum: dict) -> bytes:
    """7-byte (NUM_TM_HM=55 bit) field: bit (TMNUM-1) set per learnable move."""
    bits = bytearray(7)
    for mv in moves_list:
        if mv not in tmnum:
            raise ValueError(f"{mv} is not a TM/HM move")
        n = tmnum[mv] - 1
        bits[n // 8] |= 1 << (n % 8)
    return bytes(bits)
...
def main():
    ...
    for f in species_order():
        base += parse_species(f, types, moves, growth, tmnum)
    if len(base) != 151 * BASE_DATA_SIZE:
        sys.exit(f"BaseStats wrong size: {len(base)} != {151 * BASE_DATA_SIZE}")

    # PokedexOrder (pret data/pokemon/dex_order.asm), resolving DEX_* names.
    idx = bytearray()
    ...
    header = (
        "; base_stats.inc — generated by tools/generators/gen_base_stats.py. DO NOT EDIT BY HAND.\n"
        "; BaseStats: 151 x 28-byte records in DEX order. Sprite block (5 bytes at\n"
        "; +10) is zeroed (DOS backend, no battle pics). TM/HM bitfield (7 bytes at\n"
        "; +20) is filled from each mon's tmhm macro.\n"
    )
    ASSETS.mkdir(parents=True, exist_ok=True)
    out = header + "\n" + emit_table("BaseStats", bytes(base)) + "\n"
    (ASSETS / "base_stats.inc").write_text(out)
    print(f"  wrote {ASSETS / 'base_stats.inc'} "
          f"(BaseStats {len(base)} bytes / 151 mons)")

    dex_header = (
        "; dex_order.inc — generated by tools/generators/gen_base_stats.py. DO NOT EDIT BY HAND.\n"
        "; PokedexOrder (pret data/pokemon/dex_order.asm): internal index (1-based)\n"
        "; -> national dex number. Included by src/data/pokemon/dex_order.asm.\n"
    )
    (ASSETS / "dex_order.inc").write_text(dex_header + "\n" + emit_table("PokedexOrder", bytes(idx)) + "\n")
    print(f"  wrote {ASSETS / 'dex_order.inc'} (PokedexOrder {len(idx)} bytes)")
