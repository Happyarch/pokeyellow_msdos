...
# Raw `_Xxx` far streams to emit even though no generatable wrapper references them.
# collect_wrappers deliberately skips a `text_far` + `text_asm` wrapper (the far part
# is only an intro; the rest is chosen at runtime), so its far stream is never
# flattened into anything and would otherwise not reach the port at all. The wrapper
# itself is composed in code — but the INTRO IS STILL DATA and belongs here, not as
# hand-written charmap bytes in a .asm.
#
# _ChargeMoveEffectText — pret data/text/text_5.asm, printed by
# engine/battle/effects.asm:ChargeMoveEffectText ahead of its text_asm selector,
...
def main():
    ...
    lines = [
        "; battle_text.inc — generated by tools/generators/gen_battle_text.py. DO NOT EDIT BY HAND.",
        ...
        f"; + {len(EXTRA_FAR)} raw far stream(s) whose wrapper is composed in code.",
    ...
    for label in EXTRA_FAR:
        if label not in far:
            raise SystemExit(f"gen_battle_text: EXTRA_FAR label {label!r} not found in "
                             f"pret data/text (collect_far)")
        lines.append(fmt(label, far[label]))
