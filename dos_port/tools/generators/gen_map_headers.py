...
def main(debug_warps=None):
    ...
    # ---- Emit output ----
    # The file that DEFINES a label DECLARES it global (gen_globals.py rule):
    # these are the names consumed outside the hosting TU
    # (src/data/maps/map_headers.asm) — LoadMapHeader/LoadOverworldAssets/
    # LoadTilesetHeader reach them via extern.
    exported = [
        "MapHeaderPointers",
        "TilesetGfxPtrs", "TilesetBlocksPtrs", "TilesetCollPtrs",
        "TilesetGfxSizes", "TilesetBlocksSizes",
        "IndoorMapBlkPtrs", "IndoorMapBlkSizes",
        "map_headers_data", "MAP_HEADERS_DATA_SIZE",
    ]
    lines = [
        ...
        "",
    ] + [f"global {name}" for name in exported] + [
        "",
        "; ==========================================================================",
    ...
    # Hosted by src/data/maps/map_headers.asm (the data-layer TU), which adds
    # one line: %include "assets/extra_includes.inc"
    already_included = {
    ...
    extra_lines = [
        ...
        "; Included from src/data/maps/map_headers.asm to bring in non-overworld",
        "; tileset assets and indoor/extra outdoor map blk data.",
        "",
