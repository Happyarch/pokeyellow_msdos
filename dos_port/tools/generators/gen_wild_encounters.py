...
def main():
    ...
    # ONE OUTPUT PER PRET SOURCE FILE, so each carrier .asm can live at its
    # mirrored path (current_plan_data_path_mirror.md). WildDataPointers and the
    # per-map blobs it points at are data/wild/grass_water.asm + data/wild/maps/;
    # WildMonEncounterSlotChances is data/wild/probabilities.asm.
    out = []
    out.append("; AUTO-GENERATED by tools/generators/gen_wild_encounters.py — do not edit.")
    out.append("; Wild-encounter data (battle engine plan, Stage 9), from")
    out.append("; data/wild/grass_water.asm + the per-map blobs in data/wild/maps/.")
    ...
    # They stay with the pointer table that `dd`-references them (same object).
    emitted = set()
    for lbl in pointers:
        if lbl in emitted:
            continue
    ...
    # Encounter-slot cumulative chance table (data/wild/probabilities.asm).
    prob = []
    prob.append("; AUTO-GENERATED by tools/generators/gen_wild_encounters.py — do not edit.")
    prob.append("; WildMonEncounterSlotChances (data/wild/probabilities.asm):")
    prob.append(f"; {len(slot_chances)} (cumulative_chance-1, slot*2) pairs.")
    prob.append("")
    prob.append("WildMonEncounterSlotChances:")
    for cum, slot2 in slot_chances:
        prob.append(f"    db {cum}, {slot2}")
    prob.append("")

    ASSETS.mkdir(parents=True, exist_ok=True)
    dst = ASSETS / "wild_grass_water.inc"
    dst.write_text("\n".join(out) + "\n")
    dst_prob = ASSETS / "wild_probabilities.inc"
    dst_prob.write_text("\n".join(prob) + "\n")
    print(f"wrote {dst} (WildDataPointers {len(pointers)} entries, "
          f"{len(emitted)} unique blobs), {dst_prob} "
          f"({len(slot_chances)} encounter slots)")
