...
; Debug player/rival names (pret charmap encoding), now Tier-1 generated data.
; Copied to W_PLAYER_NAME / W_RIVAL_NAME in PrepareTitleScreen, and to the same by
; PrepareOakSpeech (oak_speech.asm shares this one contiguous block). The three
; labels MUST stay contiguous at pret's exact lengths: the NAME_LENGTH(11) copies
; deliberately overrun, so wPlayerName really holds "NINTEN@SONY" on hardware — the
; golden caught the padded-to-11 version as wrong. See
; tools/generators/gen_debug_boot_names.py for the full note.
