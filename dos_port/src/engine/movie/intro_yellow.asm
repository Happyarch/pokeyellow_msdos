...
; DEVIATION{class=HAL; pret=engine/movie/intro_yellow.asm:YellowIntroScene6; behavior=the per-scanline LY scroll override is not emulated, so the rSCY value written through hLCDCPointer never displaces any scanline and the surfing scene's water renders flat instead of waving, the port stores hLCDCPointer and wLYOverridesBuffer faithfully but nothing consumes them; evidence=this scene never publishes its overrides to the compositor - the per-scanline scroll channel now EXISTS as g_row_yoff and g_row_yoff_on in src/ppu/ppu.asm, added 2026-08-18 with the Surfing Pikachu minigame, and surfing_pikachu.asm reads hLCDCPointer and fills that table every frame, but intro_yellow writes hLCDCPointer at 9 sites and wLYOverridesBuffer here and wires neither to it, and the channel is consumed on render_window's fine path while this scene draws on the BG plane; lifetime=retire by wiring this scene to g_row_yoff, which is now a wiring job rather than a missing-capability one}
YellowIntroScene6:
    call YellowIntro_BlankPalsDelay2AndDisableLCD
    mov bl, 0x5                                    ; ld c, $5
    call UpdateMusicCTimes
