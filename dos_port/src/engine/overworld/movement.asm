; movement.asm — the overworld sprite walk/facing state machine.
;
; Mirror of pret engine/overworld/movement.asm:
;   UpdatePlayerSprite / UpdateNPCSprite / InitializeSpriteStatus /
;   InitializeSpriteScreenPosition / Func_5033 / CheckSpriteAvailability /
;   GetTileSpriteStandsOn / UpdateSpriteImage / UpdateSpriteMovementDelay /
;   UpdateSpriteInWalkingAnimation / NotYetMoving / TryWalking / CanWalkOntoTile /
;   Func_5337 / Func_5349 / Func_5357 / MakeNPCFacePlayer / ChangeFacingDirection /
;   Func_4e32 / Func_5274 / the DoScriptedNPCMovement chain
;
; WHAT IS NO LONGER HERE (chunk 17 of the relocated-label grind): this file used
; to carry the entry point and the collision engine as well, under labels whose
; pret home is elsewhere. They now sit in their own mirrors:
;   UpdateSprites                                   src/home/update_sprites.asm
;   _UpdateSprites / UpdateNonPlayerSprite /        src/engine/overworld/
;   DetectCollisionBetweenSprites /                   sprite_collisions.asm
;   SetSpriteCollisionValues
;
; The per-frame chain is unchanged: UpdateSprites -> _UpdateSprites walks all 16
; sprite slots; slot 0 -> UpdatePlayerSprite (facing + walk animation), slots
; 1-15 -> UpdateNonPlayerSprite, which dispatches into UpdateNPCSprite here (the
; full NPC walk state machine: delay countdown, 16-frame pixel-step animation,
; direction selection with UP_DOWN/LEFT_RIGHT constraints, tile passability +
; sprite-collision gating). PrepareOAMData (sprite_oam.asm) then turns the image
; indices into shadow-OAM entries each DelayFrame.
;
...
; wMapSpriteData[(slot-1)*2] (OW-A.2 P2 relocation). Reached by jne from
; sprite_collisions.asm:UpdateNonPlayerSprite (pret jp's here from its .unequal
; path — the port realizes it as the not-taken arm of the same compare). ESI is live from
