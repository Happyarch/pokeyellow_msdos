...
; DEVIATION{class=HAL; pret=engine/overworld/advance_player_sprite.asm:_AdvancePlayerSprite; behavior=the wMapViewVRAMPointer 2-tile slide and the four ScheduleSouthRowRedraw ScheduleNorthRowRedraw ScheduleEastColumnRedraw and ScheduleWestColumnRedraw calls are dropped, so this routine never moves the VRAM window pointer or stages a redraw strip, though all five of those pret labels including the ScheduleColumnRedrawHelper they share DO have faithful port bodies in src/home/overworld.asm which nothing reaches; evidence=the slide moves wMapViewVRAMPointer by 2 tiles per direction and the Schedule calls stage one 2-tile-wide strip of the 256x256 VRAM tilemap torus per frame into wRedrawRowOrColumnSrcTiles for RedrawRowOrColumn to DMA during next VBlank, and the port has no torus - render_bg in src/ppu/ppu.asm decodes wSurroundingTiles into a 48x36-tile surface and blits a 320x200 window at a signed pixel offset, so the slid window pointer is vestigial and the strip is already on screen the moment LoadCurrentMapView above rebuilds it - see docs/plans/compositor_perf.md and CLAUDE.md on torus removal; lifetime=permanent, structural to the surface renderer}
.updateMapView:
