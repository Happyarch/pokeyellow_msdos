...
extern IsInArray                        ; src/home/array2.asm — AL in $FF-array at ESI, stride EDX
...
.dakutensAndHandakutens:
    ...
    ; fallthrough shape) rather than calling the lookup. DakutensAndHandakutens
    ; itself IS ported (below) and is simply never called from here.
.addLetter:
    mov al, [ebp + wNamingScreenLetter]
    mov [ebp + esi], al
    inc esi
...
; copy: ED_Tile (16 bytes, already 1bpp->2bpp expanded by gen_alphabets.py, which
; also emits the port-side alias alphabet_ed_tile at the same address) to the
; vChars1 slot for char ALPHABET_ED_CHAR ($F0) in $8800-signed addressing.
; DEVIATION{class=data-model; pret=engine/menus/naming_screen.asm:ED_Tile; behavior=store the ED glyph pre-expanded to 2bpp (16 bytes) where pret INCBINs the raw 1bpp glyph (8 bytes), so ED_Tile is not byte-identical to pret's; evidence=gen_alphabets.py expands gfx/font/ED.1bpp exactly as LoadFontTilePatterns expands every other font tile, and the port's tile cache decodes 2bpp only; lifetime=permanent, follows the port's font storage format}
; ---------------------------------------------------------------------------
LoadEDTile:
    ...
    push edi
    push ecx
    mov esi, ED_Tile                      ; ld de, ED_Tile (alias: alphabet_ed_tile)
    lea edi, [ebp + GB_VFONT + (ALPHABET_ED_CHAR - 0x80) * TILE_SIZE]
    mov ecx, ED_TileEnd - ED_Tile         ; == ALPHABET_ED_TILE_SIZE
    rep movsb
    pop ecx
    pop edi
    pop esi
...
; ---------------------------------------------------------------------------
; DakutensAndHandakutens — pret ref: naming_screen.asm:DakutensAndHandakutens.
; Take the last character typed into wStringBuffer, look it up in the kana
; voicing table the caller passes, and if it is there replace wNamingScreenLetter
; with the voiced form. Tables: src/data/text/dakutens.asm (Dakutens /
; Handakutens), generated by tools/generators/gen_dakutens.py.
;
; UNREFERENCED in the port: the only pret call site is the
; .dakutensAndHandakutens branch above, which the EN alphabet can never reach
; (see the DEVIATION there). Ported for label-for-label parity with the pret
; file, not for behaviour.
;
; In:  EDX = flat pointer to the table (pret: DE), $FF-terminated 2-byte pairs.
; Out: CF = a substitution was made. Clobbers EAX/EBX/ECX/EDX/ESI.
; ---------------------------------------------------------------------------
DakutensAndHandakutens:
    push edx                              ; push de — the table pointer
    call CalcStringLength                 ; ESI -> '@' in wStringBuffer
    dec esi                               ; dec hl
    mov al, [ebp + esi]                   ; ld a, [hl] — last character typed
    pop esi                               ; pop hl — table pointer (FLAT, .data)
    mov edx, 2                            ; ld de, $2 — {plain, voiced} stride
    call IsInArray                        ; CF = found, ESI -> the matching pair
    jnc .ret                              ; ret nc
    inc esi                               ; inc hl
    mov al, [esi]                         ; ld a, [hl] — flat table read
    mov [ebp + wNamingScreenLetter], al
.ret:
    ret
