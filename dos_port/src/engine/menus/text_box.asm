...
; ---------------------------------------------------------------------------
; Menu strings — pret data/text_boxes.asm. Charmap bytes ('A'=$80..'Z'=$99,
; ' '=$7F, <NEXT>=$4E, '@'=$50, <PK>=$E1, <MN>=$E2, '×'=$F1, '¥'=$F0).
; DEVIATION: the JP-only strings are omitted with the JP table rows.
; ---------------------------------------------------------------------------
BuySellQuitText:                        ; "BUY" next "SELL" next "QUIT@"
    db 0x81,0x94,0x98, 0x4E
    db 0x92,0x84,0x8B,0x8B, 0x4E
    db 0x90,0x94,0x88,0x93, 0x50
    db 0x50                             ; unused stray "@" (pret keeps it)

UseTossText:                            ; "USE" next "TOSS@"
    db 0x94,0x92,0x84, 0x4E
    db 0x93,0x8E,0x92,0x92, 0x50

MoneyText:                              ; "MONEY@"
    db 0x8C,0x8E,0x8D,0x84,0x98, 0x50

BattleMenuText:                         ; "FIGHT <PK><MN>" next "ITEM  RUN@"
    db 0x85,0x88,0x86,0x87,0x93, 0x7F, 0xE1,0xE2, 0x4E
    db 0x88,0x93,0x84,0x8C, 0x7F,0x7F, 0x91,0x94,0x8D, 0x50

SafariZoneBattleMenuText:               ; "BALL×       BAIT" next "THROW ROCK  RUN@"
    db 0x81,0x80,0x8B,0x8B, 0xF1, 0x7F,0x7F,0x7F,0x7F,0x7F,0x7F,0x7F
    db 0x81,0x80,0x88,0x93, 0x4E
    db 0x93,0x87,0x91,0x8E,0x96, 0x7F, 0x91,0x8E,0x82,0x8A, 0x7F,0x7F
    db 0x91,0x94,0x8D, 0x50

SwitchStatsCancelText:                  ; "SWITCH" next "STATS" next "CANCEL@"
    db 0x92,0x96,0x88,0x93,0x82,0x87, 0x4E
    db 0x92,0x93,0x80,0x93,0x92, 0x4E
    db 0x82,0x80,0x8D,0x82,0x84,0x8B, 0x50

PokemonMenuEntries:                     ; "STATS" next "SWITCH" next "CANCEL@"
    db 0x92,0x93,0x80,0x93,0x92, 0x4E
    db 0x92,0x96,0x88,0x93,0x82,0x87, 0x4E
    db 0x82,0x80,0x8D,0x82,0x84,0x8B, 0x50

CurrencyString:                         ; "      ¥@" (unreferenced; kept per pret)
    db 0x7F,0x7F,0x7F,0x7F,0x7F,0x7F, 0xF0, 0x50

; ===========================================================================
section .bss
tb_saved_stride: resd 1                 ; caller's text_row_stride across a dispatch
...
DisplayTextBoxID_:
    ...
    ; at .done (mov only — leaves the handler's CF/AL contract intact).
    mov eax, [text_row_stride]
    mov [tb_saved_stride], eax
...
.done:
    ; restore the caller's stride; push/mov/pop preserves EFLAGS and AL so the
    ; function-table handlers' return contract (CF) passes through untouched
    push eax
    mov eax, [tb_saved_stride]
