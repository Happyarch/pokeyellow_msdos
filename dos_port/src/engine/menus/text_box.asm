...
; This is why the port does not CALL
; TwoOptionMenu_SaveScreenTiles / TwoOptionMenu_RestoreScreenTiles (ledger M-5).
; Both are ported, faithfully, at the end of this file — for label-for-label
; parity with the pret file — but nothing reaches them: pret must SNAPSHOT the
...
; ---------------------------------------------------------------------------
; TwoOptionMenu_SaveScreenTiles / TwoOptionMenu_RestoreScreenTiles — pret ref:
; engine/menus/text_box.asm, immediately below DisplayTwoOptionMenu.
;
; Snapshot the 5-row x 6-column tilemap block at ESI (pret: HL) into wBuffer, and
; paste it back. pret's own comment above them notes the sizing defect: wider or
; taller two-option menus are not fully covered by 5x6, so their bottom and right
; edges survive the restore. Carried as written.
;
; UNREFERENCED in the port: DisplayTwoOptionMenu here composites a window
; descriptor over an untouched background, so it neither saves nor restores
; tilemap cells (see the DEVIATION on yn_teardown above). Ported for
; label-for-label parity with the pret file, not for behaviour.
;
; DEVIATION{class=projection; pret=engine/menus/text_box.asm:TwoOptionMenu_SaveScreenTiles; behavior=advance to the next row by the port canvas row stride minus 6 rather than pret's literal SCREEN_WIDTH minus 6; evidence=the port wTileMap is 40x25 and publishes its active row stride as text_row_stride which every ported tilemap walker already uses, so pret's literal would skew each row; lifetime=permanent canvas projection boundary}
;
; In:  ESI = top-left tilemap cell (EBP-relative). Clobbers EAX/EBX/ECX/EDX/ESI.
; ---------------------------------------------------------------------------
TwoOptionMenu_SaveScreenTiles:
    mov edx, wBuffer                    ; ld de, wBuffer
    mov bx, 0x0506                      ; lb bc, 5, 6 — BH = rows, BL = columns
.loop:
    ...
    jnz .loop
    ; pret push/pops bc around `ld bc, SCREEN_WIDTH - 6`; the port keeps the
    ; counters in BX and does the row step in ECX, so nothing needs saving.
