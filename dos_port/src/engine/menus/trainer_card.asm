...
; pret ref: TrainerInfo_NameMoneyTimeText — "NAME/" <NEXT> "MONEY/" <NEXT> "TIME/@"
; ('/' = $F3 divider glyph, <NEXT> = $4E). Double-spaced (rows 2,4,6) to align with
; the value coords (name (7,2), money (8,4), time (9,6)) — BIT_SINGLE_SPACED_LINES
; is cleared before PlaceString so <NEXT> advances two rows (players_pc.asm model).
TrainerInfo_NameMoneyTimeText:
    db 0x8D,0x80,0x8C,0x84,0xF3, 0x4E, 0x8C,0x8E,0x8D,0x84,0x98,0xF3, 0x4E, 0x93,0x88,0x8C,0x84,0xF3, 0x50
; pret ref: TrainerInfo_BadgesText — $76 "BADGES" $76 "@" ($76 = ○ circle tile)
TrainerInfo_BadgesText:
    db 0x76, 0x81,0x80,0x83,0x86,0x84,0x92, 0x76, 0x50
...
DrawTrainerInfo:
    ...
    mov esi, tc_box_tiles
    lea edi, [ebp + GB_VCHARS2 + 0x77 * TILE_SIZE]
    mov ecx, (TC_BOX_TILE_COUNT * TC_TILE_BYTES) / 4
    rep movsd
    ; BlankLeaderNames → vChars2 $60
    mov esi, tc_blank_name_tiles
    lea edi, [ebp + GB_VCHARS2 + 0x60 * TILE_SIZE]
    mov ecx, (TC_BLANK_NAME_COUNT * TC_TILE_BYTES) / 4
    rep movsd
    ; GymLeaderFaceAndBadgeTileGraphics → vChars2 $20 (package B LoadBadgeTiles)
    call LoadBadgeTiles
    ; BadgeNumbersTileGraphics → vChars1 $58 (drawn as ids $D8+)
    mov esi, tc_badge_number_tiles
    lea edi, [ebp + TC_VCHARS1 + 0x58 * TILE_SIZE]
    mov ecx, (TC_BADGE_NUM_COUNT * TC_TILE_BYTES) / 4
    rep movsd
    ; colon glyph (TextBoxGraphics tile 13) → vChars1 $56 (id $D6)
    mov esi, tc_colon_tile
    lea edi, [ebp + TC_VCHARS1 + 0x56 * TILE_SIZE]
    mov ecx, TC_TILE_BYTES / 4
    rep movsd
    ; box background tile (trainer_info tile 8) → vChars1 $57 (id $D7)
    mov esi, tc_bg_tile
    lea edi, [ebp + TC_VCHARS1 + 0x57 * TILE_SIZE]
    mov ecx, TC_TILE_BYTES / 4
    rep movsd
    ; ○ circle → vChars2 $76 (id $76)
    mov esi, tc_circle_tile
    ...
    call PlaceString
    ; NAME/MONEY/TIME labels at (2,2) — double-spaced (rows 2,4,6)
