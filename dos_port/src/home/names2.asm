; names2.asm — mirror of pret home/names2.asm.
;
; pret home/names2.asm holds exactly two labels and this file holds both, in
; pret order:
;
;   NamePointers  (pret home/names2.asm:1)
;   GetName       (pret home/names2.asm:11)
;
; Both arrived here from src/home/names.asm, which is the mirror of the OTHER
; pret name file (home/names.asm: GetMonName / GetItemName / GetMachineName /
; IsItemHM / IsMoveHM / HMMoves / GetMoveName, plus TechnicalPrefix and
; HiddenPrefix from assets/home_names_runtime_strings.inc). The two pret files
; are one subsystem but two mirrors; the split follows pret.
;
; UnusedBadgeNames is NOT a home/ label — pret defines it in
; data/text/unused_names.asm and only NamePointers references it, so it travels
; with the table it is an entry of.
;
; The gen-1 name dispatcher. GetName resolves [wNameListIndex] within the list
; chosen by [wNameListType] (a NamePointers index) into wNameBuffer. The
; wrappers that set the type and tail-call in here (GetMonName / GetMoveName /
; GetItemName / GetMachineName) live in src/home/names.asm.
;
...
;
; BUG{class=data-model; pret=home/names2.asm:GetName; behavior=HM01 threshold redirects every name type, not only items, to machine-name formatting; evidence=pret GetName unconditional cp HM01 before type dispatch; lifetime=permanent latent Gen-1 behavior}
...
;
; Build: nasm -f coff -I include/ -I . -o names2.o src/home/names2.asm
...
extern ItemNames
extern TrainerNames
extern GetMonName               ; src/home/names.asm — fixed-width species path
extern GetMachineName           ; src/home/names.asm — TM/HM name formatter
