...
extern menu_tilemap_base        ; home/window.asm(resd) base tilemap for PlaceMenuCursor
...
LIST_SCRATCH    equ PIC_STAGE   ; 0xB860 in GB space (dedicated list scratch)
LIST_STRIDE     equ 20          ; box-relative stride (NOT port SCREEN_WIDTH=40)
...
section .data
align 4
list_scratch: dd wTileMap

; ListMenuCancelText / InitialQuantityText / SpacesBetweenQuantityAndPriceText —
; pret home/list_menu.asm. Tier-1 DATA: generated by tools/generators/gen_list_menu_strings.py
...
%endif
    ...
    mov dword [list_scratch], wTileMap
    mov dword [menu_tilemap_base], wTileMap
    cmp byte [ebp + wIsInBattle], 0
    jz .scratchReady
    mov dword [list_scratch], PIC_STAGE
    mov dword [menu_tilemap_base], PIC_STAGE
.scratchReady:
...
DisplayListMenuIDLoop:
    ...
    ; place ▶ at box-rel cursor and auto-advance.
    mov eax, [list_scratch]
    mov byte [ebp + eax + LIST_NAME_ROW0 * LIST_STRIDE + LIST_CURSOR_COL], CHAR_CURSOR
    call list_mirror
    mov bl, 20
    call DelayFrames
    xor al, al
    mov [ebp + wCurrentMenuItem], al
    ; wMenuCursorLocation = box-rel cursor address (low/high byte)
    mov eax, [list_scratch]
    add eax, LIST_NAME_ROW0 * LIST_STRIDE + LIST_CURSOR_COL
...
%endif
    ...
    ; Tell HandleMenuInput where this menu's "more below" ▼ actually is.
    mov eax, [list_scratch]
    add eax, LIST_DOWN_ROW * LIST_STRIDE + LIST_DOWN_COL
    mov dword [menu_arrow_pos], eax
...
.drawTextBox:
    ...
    mov esi, [list_scratch]
    add esi, QTY_SROW * LIST_STRIDE + 1 * LIST_STRIDE + 1
...
.skipMul:
    ...
    mov esi, [list_scratch]
    add esi, LIST_NAME_ROW0 * LIST_STRIDE + LIST_NAME_COL
...
.nextListEntry:
    ...
    mov eax, [list_scratch]
    mov byte [ebp + eax + LIST_DOWN_ROW * LIST_STRIDE + LIST_DOWN_COL], CHAR_DOWN
    ret
...
; ----------------------------------------------------------------------------
; Window helper — register the list box descriptor at the overworld-ui or battle anchor.
; Mirrors bag_menu.asm .add_list_window.
; ----------------------------------------------------------------------------
list_draw_box_border:
    mov esi, [list_scratch]                    ; box top-left in scratch
    mov bl, LIST_INT_W
    mov bh, LIST_INT_H
    call TextBoxBorder
...
list_add_qty_window:
    ...
    cmp byte [ebp + wIsInBattle], 0
    jz .checkMart
    mov eax, QTY_WX_BATTLE
    mov ebx, QTY_WY_BATTLE
    mov edx, QTY_MAXY_BATTLE
    jmp .publish
.checkMart:
...
; Mirror helpers — copy the staged boxes from the list scratch (stride 20)
...
list_clear_interior:
    ...
    push edx
    push edi
    mov edi, [list_scratch]
    lea edi, [ebp + edi + 1 * LIST_STRIDE + 1]
