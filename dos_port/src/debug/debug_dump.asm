...
%ifdef DEBUG_TRAINER_ROUTE17
    ...
    ; Same rule as DEBUG_TRAINER_ROUTE above: scenario-local, mirrored by
    ; tools/mgba_harness/scenarios/route17_trainer_battle.lua, joined by NAME.
    ;
    ; UNLIKE DEBUG_TRAINER_ROUTE, this dump fires at BATTLE ENGAGEMENT, not after
    ; the fight closes — "engage a trainer" is the acceptance bar this scenario
    ; exists to clear, and stopping there avoids re-deriving a lethal move for a
    ; different roster. wPlayerMapPos.wYCoord is the direct witness: it is exactly
    ; the byte that diverged in both earlier route17_sight attempts ($79 vs $78)
    ; because RunMapScriptSightTest could never run ForceBikeDown's forced
    ; southward drift, and it is confirmed byte-identical to that measurement here
    ; (0x79) with the real loop driving it.
    ; wTrainerEngageDistance/screenY are DELIBERATELY NOT compared here: measured
    ; live (2026-08-15), by the time the battle has actually started that WRAM
    ; span is scratch reused by other systems (distance read back as 0x40, an
    ; impossible tile distance) — it is only meaningful during the SIGHT check
    ; itself, which route*_sight already covers. Comparing it post-engagement
    ; would risk a spurious divergence from unrelated scratch reuse, not from
    ; ForceBikeDown.
    gbregion "wPlayerMapPos",    wCurMap, 5                ; wCurMap .. wXCoord
    gbregion "wIsInBattle",      wIsInBattle, 1
    gbregion "wCurOpponent",     wCurOpponent, 1
...
windows:
    ...
    ; (W_PORT_SCRATCH + 0x51) so the PAS block is visible. Post-expansion
    ...
    ; them). Covers GB 0xF551..0xF590: MIDI CC7[3..15] (+0x51..5D), innova
    ; V1 (+0x5E..5F), pika PCM (+0x60..65), hal device (+0x66..67), tandy
    ; (+0x68..6F), speaker (+0x70..77), OPL enh (+0x78..7C), innova V2/V3 +
    ; lastmode (+0x7D..7F), W_CHECK_FOR_TURN (+0x80), Covox DAC (+0x81..88,
    ; file 0x230..0x237), CMS (+0x89..8C, file 0x238..0x23B), PAS (+0x8D..90,
    ; file 0x23C..0x23F). The MIDI head (+0x4D scale, +0x4E..50 CC7[0..2])
    ; stays undumped — one 64-byte window cannot span +0x4D..+0x90 — as does
    ; the opl SB-detect/MIDI head (+0x40..4C) before it.
...
%ifdef DEBUG_TRAINER_ROUTE17
; ---------------------------------------------------------------------------
; RunTrainerRoute17TestSeed — ROUTE_17 / ForceBikeDown witness (map-script
; fidelity plan, third attempt at wiring ROUTE_17).
;
; Same shape as RunTrainerRouteTestSeed: SEEDS AND RETURNS. EnterMap falls
; through into the real OverworldLoop, which is the only loop that ever calls
; ForceBikeDown (called from OverworldLoopLessDelay, just before
; AreInputsSimulated) — RunMapScriptSightTest structurally cannot reach it, which
; is why the two earlier route17_sight attempts could never witness the routine
; even after it was ported (regression-overworld-forcebikedown-missing).
;
; Arms ROUTE17_BIKER10 (data/maps/objects/Route17.asm, OPP_BIKER 12,
; Route17TrainerHeader9) the way a fresh arrival would: persistent beaten bit
; clear, map script at its DEFAULT handler.
;   wRoute17CurScript = 0xD61B (derived by tools/generators/gen_map_script_
;     tables.py's parse_cur_script_addresses walk from wGameProgressFlags;
;     cross-checked against wRoute3CurScript = 0xD5F7, which IS in
;     include/gb_memmap.inc, so the walk is trustworthy for a map that isn't).
...
;
; Seeds the debug party (standard-region parity with the golden's
; seed.debug_new_game) and an empty bag — this flow uses no items. Spawn coords
; are seeded earlier, in EnterMap (see the DEBUG_TRAINER_ROUTE17 block in
; src/home/overworld.asm).
;
; ONE-SHOT guard kept for symmetry with RunTrainerRouteTestSeed even though this
; scenario's dump fires at battle-engagement (state-gated in AutoKeyDrive, well
; before any post-battle EnterMap re-entry could occur) — cheap insurance against
; a re-seed if the dump gate is ever mistuned.
;
; In: EBP = GB memory base.  Returns.
; ---------------------------------------------------------------------------
RunTrainerRoute17TestSeed:
    cmp byte [trroute17_seeded], 0
    jne .alreadySeeded17
    mov byte [trroute17_seeded], 1
    mov byte [ebp + wPartyCount], 0
    mov byte [ebp + wPartySpecies], 0xFF
    mov byte [ebp + wNumBagItems], 0
    mov byte [ebp + wBagItems], 0xFF
...
section .bss
trroute17_seeded: resb 1                        ; 0 = seed on first EnterMap only
section .text
%endif
...
%endif
%ifdef DEBUG_TRAINER_ROUTE17
    ; State-gated photograph: the trainer battle is CONFIRMED STARTED (wIsInBattle
    ; == 2, trainer, AND wEnemyMonSpecies != 0 — the loader has run, so this is
    ; not the stale-nonzero window AUTOKEY_DUMP_ON_BATTLE's comment warns about).
    ; That is the acceptance bar this scenario exists to clear: the map script
    ; engaged the trainer through the REAL OverworldLoop, driven by ForceBikeDown's
    ; forced southward drift exactly as ground truth does. Fighting the battle out
    ; is deliberately NOT this scenario's job (trainer_battle_route already proves
    ; the live menu/turn-loop/return choreography on Route 3); waiting a short
    ; settle delay after send-out lets both sides finish the intro slide before the
    ; photograph, the same reasoning as AUTOKEY_DUMP_ON_BATTLE's delay.
%ifndef AUTOKEY_R17_DUMP_DELAY
%define AUTOKEY_R17_DUMP_DELAY 60
%endif
    cmp byte [ebp + wIsInBattle], 2
    jne .noR17Dump
    cmp byte [ebp + wEnemyMonSpecies], 0
    je .noR17Dump
    inc dword [autokey_r17_battle_frame]
    mov eax, [autokey_r17_battle_frame]
    cmp eax, AUTOKEY_R17_DUMP_DELAY
    jne .noR17Dump
    call DumpBackbuffer                 ; GBSTATE.BIN + FRAME.BIN, then exits
.noR17Dump:
%endif
...
%ifdef AUTOKEY_TRAINER_ROUTE17
    ...
    ; still walking up. *** home/trainers.asm's per-frame NPC-dialog dispatch
    ; (src/engine/overworld/map_sprites.asm TALK sentinel) runs TalkToTrainer for
    ; ANY A press that lands while the player faces an adjacent trainer sprite —
    ; not just a deliberate "talk" — and TalkToTrainer sets wStatusFlags7
    ; BIT_USE_CUR_MAP_SCRIPT unconditionally, before it checks whether the
    ; trainer is already sight-engaged. During TrainerWalkUpToPlayer's approach
    ; (wStatusFlags5 BIT_SCRIPTED_NPC_MOVEMENT set) an A press held on exactly the
    ; wrong frame can therefore fire it. DisplayEnemyTrainerTextAndStartBattle
    ; will not print (and so needs no A to dismiss) until that same bit clears,
    ; so withholding A for its whole duration costs nothing — it only delays the
    ; cadence's next scheduled press to the following window, and this scenario's
    ; dump is state-gated on battle start, not on a frame count.
...
%ifdef DEBUG_TRAINER_ROUTE17
align 4
autokey_r17_battle_frame: dd 0         ; frames counted after trainer battle engaged
%endif
autokey_prev:  db 0
align 4
...
%endif
    dd   -1,   -1, 0
%elifdef AUTOKEY_TRAINER_ROUTE17
    ; ROUTE_17 / ForceBikeDown witness cadence.
    ;
    ; Unlike AUTOKEY_TRAINER_ROUTE, this script presses ONLY A, and only in short
    ; pulses — never the D-pad. ForceBikeDown's guard (src/home/overworld.asm) is
    ; `hJoyHeld & (PAD_CTRL_PAD|PAD_B|PAD_A) == 0 -> force DOWN`, so any D-pad
    ; press here would suppress the very auto-scroll this scenario exists to
    ; witness. An A pulse only blocks ForceBikeDown for the frames it is held,
    ; which is harmless: the dump (AUTOKEY_DUMP_ON_TRAINER_ROUTE17 below) is
    ; state-gated on the trainer battle actually starting, not on a fixed frame,
    ; so a few suppressed frames only delay the dump, never skip it.
    ;
    ; The pulses exist only to dismiss whatever pre-battle text TrainerEngage
    ; prints (the biker's approach line) — an A with no dialog open is a no-op on
    ; the overworld, same reasoning as AUTOKEY_TRAINER_ROUTE's cadence comment.
    ;
    ; SPARSE BY DESIGN (MEASURED 2026-08-15, two cadences tried): a dense cadence
    ; (matching the golden Lua's original input.tap("A", 2, 8) — a press every 8
    ; frames) suppresses ForceBikeDown often enough to change the FINAL drift
    ; distance (wYCoord landed one tile short, $78 instead of the ground-truth
    ; $79), which is the opposite of what this scenario exists to prove. The
    ; sparse 90-frame cycle below reproduces ground truth's wYCoord exactly.
    ; See route17_trainer_battle.lua for the matching golden-side density fix
    ; (wStatusFlags7 BIT_USE_CUR_MAP_SCRIPT — an input-timing-coupled TalkToTrainer
    ; artifact, not a game-logic gap; resolved by making the GOLDEN'S cadence
    ; sparse too, rather than densifying the port's).
%ifndef AK_ROUTE17_CYCLES
%define AK_ROUTE17_CYCLES 40             ; 40 * 90 = 3600 frames of margin
%endif
%assign ak17_i 0
%rep AK_ROUTE17_CYCLES
    dd 60 + ak17_i * 90, 68 + ak17_i * 90, PAD_A
%assign ak17_i ak17_i + 1
%endrep
    dd   -1,   -1, 0
